		...
		success:function(data){
			var obj=eval('('+data+')');
		 	var str="<ul  style='text-align:left'>";
		   	for(var i=0;i<obj.length;i++){
		   		str+="<li style='padding-left:5px' "
	   			str+="  onclick='examintion_item_click2(this,"+obj[i].id+");'  >"+obj[i].dep_name+"</li>"
		   	}
		   	str+="</ul>";
		   	$(ob).html(str); 
		}
...
function  adddisease_logic(){
	...
	$("#divshou dl").each(function(){
	 ...
	 $.ajax({
		url:'saveDiseaseDepLogic.action',
		data:model,
		type:'post',
		success:function(data){
...
function getDisease_exam_item(id){
	$.ajax({
		url:'getDiseaseDepLogicItem.action?id='+id,
		cache:false, 
		async:false,
		type:'post',
		success:function(data){
	...
	<legend><strong>逻辑条件设置</strong></legend> 
		<div  class="formDiv" style="background: #d9d9d9;font-size:14">
			<dl style="background: #cccccc;">
				<dt style="width:20%;text-align: center;">科室</dt>
