...
function diesaseExamination_item(ths){
	var model = {c_item_name:$(ths).val(),dep_id:$("#zdep_id").val()}
	$.ajax({
		...
		success:function(data){
			var obj=eval('('+data+')');
		 	var str="<ul style='text-align:left'>";
		   	for(var i=0;i<obj.length;i++){
		   		str+="<li style='padding-left:5px' "
	   			str+='onclick="examintion_item_click2(this,\''+obj[i].item_code+'\');">'+obj[i].item_name+'</li>'
		   	}
		   	str+="</ul>";
...
function  adddisease_logic(){
	if($("#exam_item_id_type").val()==""||$("#exam_item_id").val()<1){
		$.messager.alert('提示信息','请选择收费项目！','error');
...
function getDisease_exam_item(id){
	$.ajax({
		url:'getDiseaseLogicSingleById.action?ids='+id+'&logic_class=2',
		cache:false, 
		async:false,
		type:'post',
		success:function(data){
