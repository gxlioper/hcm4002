...
//	    		 logic_type:$('#c_logic_type').combobox('getValue'),
	     ...
	     $("#groupusershow").datagrid({
			 url:'getDiseaseLogicCompositeList.action',
			 queryParams:model,
			 rownumbers:false,
		     pageSize:15,//
...
function f_ty(id)
{
	$.messager.confirm('提示信息','是否确认停用该条记录？',function(r){
		if(r){
			$.ajax({
         	url:'diseaseLogicCompositeStartOrEnd.action?ids='+id+'&isActive=N',
         	type:'post',
...
{
	$.ajax({
		url:'diseaseLogicCompositeStartOrEnd.action?ids='+id+'&isActive=Y',
		type:'post',
		success:function(data){
			$.messager.alert('提示信息',data);
			$('#groupusershow').datagrid('reload');
 ...
 */
 function updataSampleReportDemoPage(id){
		...
		$("#dlg-custedit").dialog({
			title:'修改复合疾病自动生成逻辑',
			center:'center',
			href:'getDiseaseLogicCompositeEditPage.action?ids='+id+'&dep_id='+dep_id+'&ran='+ Math.random()
		});
...
function f_deluser(id) {
	$.messager.confirm('提示信息', '确定删除疾病逻辑？', function(r) {
		if (r) {
			$.ajax({
				url : 'delDiseaseLogicComposite.action?ids='+ id,
				type : 'post',
				success : function(data) {
					$.messager.alert('提示信息', data);
					$('#groupusershow').datagrid('reload');
...
function deluserrow(ids){
	...
	$.messager.confirm('提示信息','是否确定删除疾病逻辑',function(r){
	 	if(r){
			 ...
			 $("body").prepend(str);
			 $.ajax({
				 url : 'delDiseaseLogicComposite.action?ids='+ids,
				type : "post",
				success : function(data) {
					$(".loading_div").remove();	
...
var toolbar = [ {
	...
	handler : function() {
		...
		$("#dlg-custedit").dialog({
			title : '新增复合疾病自动生成逻辑',
			center:'center',
			href : 'getDiseaseLogicCompositeEditPage.action?dep_id='+obj.id,
		});
		$("#dlg-custedit").dialog('open');
		$("#dlg-custedit").dialog('center');
	}
