...
function getDiseaseKnowloedge(obj){
	var model = {disease_type:'D',disease_name:$(obj).val(),dep_id:$("#zdep_id").val()}
	$.ajax({
...
function diesaseExamination_item(ths){
	if($(ths).parent().parent().children().eq(0).children().eq(0).val() == 1){
		...
		var model = {disease_type:'Y',disease_name:$(ths).val(),dep_id:$("#zdep_id").val()}
		$.ajax({
			url:'getDiseaseKnowloedge.action',
			data:model,
			type:'post',
			success:function(data){
				var obj=eval('('+data+')');
			 	var str="<ul  style='text-align:left'>";
			   	for(var i=0;i<obj.length;i++){
			   		str+="<li   style='padding-left:5px' "
		   			str+='onclick="examintion_item_click2(this,\''+obj[i].disease_num+'\')">'+obj[i].disease_name+'</li>'
			   	}
			   	str+="</ul>";
			   	$(ths).parent().children().eq(2).html(str);
			   	$(ths).parent().children().eq(2).show();
...
function  adddisease_logic(){
	...
	$(".tiaojian_id").each(function(){
		...
		$(this).children().eq(2).children().each(function(){
			conditions.push({
				'diseaseOrItem_num':$(this).children().eq(1).children().eq(0).val(),
				'condition_type':$(this).children().eq(0).children().eq(0).val(),
	...
	$.ajax({
		url:'saveDiseaseLogicComposite.action',
		data:model,
		type:'post',
		success:function(data){
...
function add_tioajian(){
	...
	var str = '<fieldset class="tiaojian_id" style="margin-left:10px;height:270px;width:480px;float:left;">'
			 ...
			 +'<dl style="background: #cccccc;"><dt style="width: 80px;">条件类型</dt><dt style="width: 150px;">检查项目/阳性</dt><dt style="width:30px;text-align: center;">条件</dt>'
			 ...
			 +'<dd><select style="width:79px;height:28px;" onchange="changItemOrDisease(this)"><option value="1">检查项目</option><option value="0">单项阳性</option></select></dd>'
			 ...
			 +'<div style="overflow-y:auto;max-height:200px;border:solid 1px #3399cc;background:#ffffff;display:none;width:144px;marign 0px" onmouseout="examintion_item_blur(this);" onmouseover="examintion_item_focus(this);"></div></dd>'
...
function changItemOrDisease(obj){
	if($(obj).val() == 0){//单项阳性类型
		$(obj).parent().parent().children().eq(1).css('width','330px');
		$(obj).parent().parent().children().eq(1).children().eq(1).css('width','330px');
		$(obj).parent().parent().children().eq(1).children().eq(2).css('width','330px');
		
		$(obj).parent().parent().children().eq(1).children().eq(0).val('');
		$(obj).parent().parent().children().eq(1).children().eq(1).val('');
		$(obj).parent().parent().children().eq(2).children().eq(0).val('>');
		$(obj).parent().parent().children().eq(3).children().eq(0).val('');
		$(obj).parent().parent().children().eq(2).hide();
		$(obj).parent().parent().children().eq(3).hide();
	}else{//检查项目结果类型
		$(obj).parent().parent().children().eq(1).css('width','150px');
		$(obj).parent().parent().children().eq(1).children().eq(1).css('width','144px');
		$(obj).parent().parent().children().eq(1).children().eq(2).css('width','144px');
...
function getDisease_exam_item(id){
	$.ajax({
		url:'getDiseaseLogicCompositeById.action?ids='+id,
		cache:false, 
		async:false,
		type:'post',
		success:function(data){
			...
			for(i=0;i<obj.logicItem.length;i++){
				...
				for(j=0;j<logicItem.itemCondition.length;j++){
					...
					$(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(0).children().eq(0).val(logicItem.itemCondition[j].condition_type);
					changItemOrDisease($(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(0).children().eq(0));
					$(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(1).children().eq(0).val(logicItem.itemCondition[j].diseaseOrItem_num);
					$(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(1).children().eq(1).val(logicItem.itemCondition[j].item_name);
					$(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(2).children().eq(0).val(logicItem.itemCondition[j].condition);
					$(".tiaojian_id").eq(i).children().eq(2).children().eq(j).children().eq(3).children().eq(0).val(logicItem.itemCondition[j].condition_value);
				}
...
<input type="hidden" id='zdep_id' value="<s:property value='dep_id'/>"  />
 <fieldset style=" margin: 10px;padding-left: 0px">
	<legend><strong>复合疾病逻辑编辑</strong></legend> 
	<div class="user-query">
		<dl>
			...
			<dd>
				...
				<div id="jianchaxiangmu" style="overflow-y:auto;height:200px;border:solid 1px #3399cc;position:relative;position:absolute;z-index:8;background:#ffffff;display:none;width:400px;marign 0px" onmouseout="examintion_item_blur(this);" onmouseover="examintion_item_focus(this);"></div>
			</dd> --%>
			<dt>生成阳性：</dt>
			<dd>
				<input type="hidden" id='disease_id' value="<s:property value='disease_id'/>"/>
				<input type="text" onblur="disease_click(this);" id="disease_id_type" onclick="getDiseaseKnowloedge(this);" onkeyup="getDiseaseKnowloedge(this);" class="textinput" style="width:550px;"/>
				<div id="disease_xlk" onmouseout="disease_blur(this);" onmouseover="disease_focus(this);" style="width:550px; position:absolute;overflow-y:auto;max-height:150px;border:solid 1px #3399cc;z-index:15;background:#ffffff;display:none;marign 0px"></div>
			</dd>
		</dl>
		<dl>
			<dt>适用性别：</dt>
