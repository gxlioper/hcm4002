...
(function(R) {
	...
	if ($.$crossFrame) {
		try {
			while (U.parent[O] != U[O]
					&& U.parent[O][C]("frameset").length == 0)
				U = U.parent
		} catch (P) {
		}
	}
	...
	function D($) {
		...
		if (Q && !Q.eCont && Q.dd && Q.dd.style.display == "block"
				&& src != Q.el)
			Q.cal.close()
	}
	function X() {
	...
	function T(E, $) {
		...
		if ($) {
			...
			if (!E.el
					|| E.el
					&& E.el.disabled
					|| (E.el == Q.el && Q.dd.style.display != "none" && Q.dd.style.left != "-1970px"))
				return;
			K(E)
		}
		function D() {
			if (S && U != W && U[O].readyState != "complete")
				return false;
			return true
		}
		function A() {
	...
	function K(E, _) {
		...
		for (D in E)
			if (Q[D] === undefined)
				Q.errMsg = "invalid property:" + D;
			else
				Q[D] = E[D];
		Q.elProp = Q.el && Q.el.nodeName == "INPUT" ? "value" : "innerHTML";
		if (Q.el[Q.elProp] == null)
			return;
		if (Q.lang == "auto")
			Q.lang = S ? navigator.browserLanguage.toLowerCase()
					: navigator.language.toLowerCase();
		if (!Q.dd
				|| Q.eCont
				|| (Q.lang && Q.realLang && Q.realLang.name != Q.lang
						&& Q.getLangIndex && Q.getLangIndex(Q.lang) >= 0)) {
			if (Q.dd && !Q.eCont)
				U[O].body.removeChild(Q.dd);
			if ($.$dpPath == "")
				F(A);
			var B = "<iframe src=\""
					+ $.$dpPath
			...
			if (Q.eCont) {
				...
				Q.dd.style.cssText = "position:fixed;z-index:19700";
				Q.dd.innerHTML = B;
				U[O].body.appendChild(Q.dd);
				Y(Q.dd.childNodes[0], X);
				if (_)
					Q.dd.style.left = Q.dd.style.top = "-1970px";
				else {
					Q.show();
					C()
				}
		...
		function C() {
			...
			if (C != Q.srcEl
					&& (C.style.display == "none" || C.type == "hidden"))
				C = Q.srcEl;
