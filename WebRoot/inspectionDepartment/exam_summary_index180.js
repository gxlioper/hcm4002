...
//				 "customer_type_id":customer_type_id
		 ...
		 url:'getHasFinalExamInfoList.action',
		 dataType: 'json',
		 queryParams:model,
		 toolbar:'#toolbar',
 ...
 function f_showreport(val,row){
	...
	function showreport(exam_num){
		if($("#report_print_type").val() == '5'){
			//预览职业病报告
			var exeurl="reportServices://&0&"+exam_num+"&Y&0";//0代表体检号Y代表预览，2代表职业病
   ...
   function open_exam_summary(exam_num){
	   var url='getExamSummaryPage.action?exam_num='+exam_num+'&operation_type=1';
   ...
   function getexamInfoGrid(){
	   ...
	   url:'getFinalExamInfoList.action',
	   dataType: 'json',
	   pageSize: 15,//每页显示的记录条数，默认为10 
	   pageList:[15,30,45,60,100],//可以设置每页记录条数的列表 
	   columns:[[
			    ...
			    {align:'center',field:'exam_types',title:'体检类型',width:15},	
			 	...
			 	{align:'center',field:'age',title:'年龄',width:10,sortable:true},
			 	{align:'center',field:'phone',title:'电话',width:20},
			 	{align:'center',field:'data_name',title:'体检类别',width:20},
...
window.setInterval(getfinalCount,20*1000);
...
//	var obj = $("#getexamInfoList").datagrid('getRows');
//  	if(obj.length > 0){
//  		$.messager.alert("操作提示", '存在未总检人员，不能继续获取总检人员!', "error");
//  		return;
//  }
  	var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
	$("body").prepend(str);
...
function searcher(value){
	...
	$("body").prepend(str);
  	$.ajax({
	    	url:'canFinalExamInfoByExamNum.action',
	    	data:{"exam_num":value},
	    	type:'post',
	    	success:function(data){
	    		$(".loading_div").remove();
