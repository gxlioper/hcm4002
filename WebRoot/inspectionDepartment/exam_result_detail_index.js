	   ...
	   $.ajax({
			...
			success : function(text) {
					if (text == '1') {//健康体检
						 ...
						 var url='getExamResultDetail.action?exam_num='+exam_num;
						 newwin = window.open("", "健康体检检验科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
						 newwin.moveTo(0,0);
						 newwin.resizeTo(screen.width,screen.height-30);
						 newwin.location = url;
						 newwin.focus();
					}else if(text == '2' || text == '3'){//职业病 或 健康体检和职业病
						$("#exam_num").textbox("setValue", "");
						 var url='getZybExamResultDetail.action?exam_num='+exam_num;
						 newwin = window.open("", "职业病体检检验科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
						 newwin.moveTo(0,0);
						 newwin.resizeTo(screen.width,screen.height-30);
						 newwin.location = url;
						 newwin.focus();
...
//						$("#exam_num").textbox("setValue", "");
//						var url='getExamResultDetailAllPage.action?examinfo_id='+id+'&exam_num='+exam_num;
//						 newwin = window.open("", "健康体检和职业病体检检验科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
//						 newwin.moveTo(0,0);
//						 newwin.resizeTo(screen.width,screen.height-30);
//						 newwin.location = url;
//						 newwin.focus();
...
function f_showitem(val,row){
	return '<a href="javascript:void(0)" onclick = "showexamitem('+row.exam_num+',this)">'+row.user_name+'</a>';
}
 //特殊人员
...
function showexamitem(exam_num,ths){
	...
	$("#examitem_div").datagrid({
		...
		dataType: 'json',
		queryParams:{'exam_num':exam_num,"exam_status":$("input[name='exam_status']:checked").val()},
		rownumbers:false,
		width:400,
		height:300,
		columns:[[
