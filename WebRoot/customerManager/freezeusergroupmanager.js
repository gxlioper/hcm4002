...
function getteamExamInfoShowGrid() {
	...
	var chk_value ="";    
	  ...
	  var exam_status = new Array();
	  var obj = $("#exam_status").combobox('getValues');
	  for(i=0;i<obj.length;i++){
		  exam_status.push("'"+obj[i]+"'");
	  }
	var model = {
		...
		"address":$("#rylb").combobox('getValues').toString(),
		"exam_status":exam_status.toString(),
		"levels":document.getElementsByName("levelss")[0].value,
		"time3":$("#time3").datebox('getValue'),	
		"time4":$("#time4").datebox('getValue'),
		"status":document.getElementsByName("freezb")[0].value,
		"chkItem":chk_value
	};
	$("#teamExamInfoShow").datagrid({
		url:'getfreezeExamInfoUserList.action',
		dataType : 'json',
		queryParams : model,
		rownumbers : true,
		...
		columns : [[ 
		             ...
		             {align : 'center',field : 'exam_num',title : tjhname,width : 18}, 
		              ...
		              {align : 'center',field : 'com_name',title : '单位',width : 47}, 
...
function termtrjndo(ids){
		 ...
		 $.messager.confirm('提示信息','是否确定对所选人员执行冻结操作？',function(r){
			 	if(r){
			 		var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
					 $("body").prepend(str);
		 $.ajax({
				url : 'examinfofreezedo.action',
				data : {ids:ids},
						type : "post",//数据发送方式   
