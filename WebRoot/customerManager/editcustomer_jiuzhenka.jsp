...
$(document).ready(function() {	 
	$('#visit_no_j').textbox('textbox').bind('blur', function() {  
		jiuzhengkayanzhen.alertyanzheng();
    });	
	 f_getDatadic();	
	 $("#birthday").datebox({
	 ...
	 $('#group_id').next().children(':first').click(function () {
		...
		exam_indicators[0].id=$("#oldgroup_id").val();
		exam_indicators[0].group_name=$("#oldgroup_name").val();		
		$("#group_id").combobox('clear'); 
		$("#group_id").combobox("loadData", exam_indicators);
		$("#group_id").combobox('setValue', $("#oldgroup_id").val());	 
	});
	...
	if(Number($("#company_id").val())<=0){
	 ...
	 $.ajax({
			...
			data : {	
						...
						sex : document.getElementsByName("sexlist")[0].value,
						is_marriage : document.getElementsByName("is_Marriagelist")[0].value,
						others:'',
...
function selectidnum(){
	...
	if(idnum.length==18){
		...
		if (!isSZ(ssidnum)) {
		  ...
		  $('#sexlist').combobox('setValue',IdCard(idnum,2));
		}
	}
}
...
function IdCard(UUserCard,num){
			...
			onLoadSuccess : function(data) {
				for (var i = 0; i < data.length; i++) {
					if (data[i].id == sextype) {
						$('#sexlist').combobox('setValue', data[i].id);
						break;
					} else {
						$('#sexlist').combobox('setValue', data[0].id);
					}
				}
			}
		});
			...
			onLoadSuccess : function(data) {
				for (var i = 0; i < data.length; i++) {
					if (data[i].id == customer_type_idtype) {
						$('#rylblist').combobox('setValue', data[i].id);
						break;
					} else {
						$('#rylblist').combobox('setValue', data[0].id);
					}
				}
			}
		});
			...
			onLoadSuccess : function(data) {
				for (var i = 0; i < data.length; i++) {
					if (data[i].id == is_Marriagetype) {
						$('#is_Marriagelist').combobox('setValue', data[i].id);
						break;
					} else {
						$('#is_Marriagelist').combobox('setValue', data[0].id);
					}
				}
			}
		});
	...
	function checkinput() {
		...
		if (document.getElementById("addcustname").value == '') {
			...
			alert('年龄不能小于1岁！');
			return false;
		}else if (document.getElementsByName("sexlist")[0].value=='') {
			alert('性别不能为空！');
			return false;
		...
		if (checkinput()) {
			...
			$.ajax({
				...
				data : {
							...
							customer_type_id:document.getElementsByName("rylblist")[0].value,
				    ...
				    $.ajax({
						...
						data : {
								...
								success : function(text) {
									...
									if (text == '1') {
										//可以正常覆盖
										if(data.sex!=document.getElementsByName("sexlist")[0].value){
	...
	function setexaminfoall(data,certno){
		...
		$("#sexlist").textbox('setValue',data.sex);
...
<input type="hidden" id="oldgroup_id" value="<s:property value="model.group_id"/>"> 
<input type="hidden" id="oldgroup_name" value="<s:property value="model.group_name"/>"> 
<input type="hidden" id="idnum_notnull" value="<s:property value="model.idnum_notnull"/>"> 
<fieldset style="margin: 5px; padding-right: 0;">
	<legend>
		<strong>体检信息编辑</strong>
	</legend>
...
<fieldset style="margin: 5px; padding-right: 0;">
	...
	<div class="user-query">
		<dl>
			<dt>就诊卡</dt>
			<dd>
			  	<input  type="hidden" id="visit_no_h"   value="<s:property value="model.visit_no" />"/>
				<input class="easyui-textbox" type="text" id="visit_no_j" 
					style="height: 26px; width: 140px;" value="<s:property value="model.visit_no" />"/>
			</dd>
		</dl>
		<dl>
			<dt>
		...
		<dl>
			...
			<dd>
				<select class="easyui-combobox" id="sexlist" name="sexlist"
					data-options="height:26,width:140,panelHeight:'auto'"></select>
			</dd>
			<dt>婚否</dt>
			<dd>
				<select class="easyui-combobox" id="is_Marriagelist" name="is_Marriagelist"
					data-options="height:26,width:140,panelHeight:'auto'"></select>
			</dd>
			
			<dt>年龄<strong class="quest-color">*</strong></dt>
			<dd>
				<input class="easyui-textbox" type="text" id="age" data-options="events:{blur:selectage}"
		...
		<dt>费别</dt>
			...
			<dt>人员类别</dt>
			<dd><select class="easyui-combobox" id="rylblist" name="rylblist"
					data-options="height:26,width:140,panelHeight:'auto'"></select>
			</dd>
		</dl>
		<dl>
		...
		<dl>		    
			...
			<dd>
				<input class="easyui-textbox" type="text" id="remark"
					style="height: 26px; width: 390px;"  value="<s:property value="model.remark" />"/>
			</dd>
			<dt>分组序列</dt>
			<dd>
