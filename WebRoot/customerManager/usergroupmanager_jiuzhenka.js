 ...
 function getgroupuserGrid(){
	 ...
	 for(i=0;i<statuss.length;i++){
		 ...
		 columns:[[
		    ...
		    {align:'center',field:'visit_no',title:'就诊卡',width:25,sortable:true},
	    		 ...
	    		 $.ajax({
	 				...
	 				success : function(data) {
	 			    	...
	 			    	for(i=0;i<temp.rows.length;i++){
	 				    	...
	 				    	obj.group_name = temp.rows[i].group_name;
	 				    	obj.set_name = temp.rows[i].set_name;
	 				    	obj.join_date = temp.rows[i].join_date;
	 				    	obj.position = temp.rows[i].position;
	 				    	obj.lis = temp.rows[i].lis;
	 				    	obj.pacs = temp.rows[i].pacs;
	 				    	obj.remark1 = temp.rows[i].remark1;
	 				    	filelist.push(obj);
	 			    	}
	 			    	$.ajax({
