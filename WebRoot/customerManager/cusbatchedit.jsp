...
function f_getDatadic() {
	...
	var is_showamount_type='<s:property value="model.is_showamount"/>';
	$('#is_showamount').combobox({
		url:'getDatadis.action?com_Type=DYXS', 
        editable:false, //不可编辑状态
        cache: false,
        panelHeight: 'auto',//自动高度适合
        valueField:'id',   
        textField:'name',
           ...
           for(var i=0;i<data.length;i++)
            {
               if(data[i].id==is_showamount_type){
            	$('#is_showamount').combobox('setValue',data[i].id);
            	break;
               }else{
            	$('#is_showamount').combobox('setValue',data[0].id);
               }
            }
        }
	});	
	
	var is_showsealtype = '<s:property value="model.is_showseal"/>';
	$('#is_showseal').combobox({
		url:'getDatadis.action?com_Type=DYXS', 
        editable:false, //不可编辑状态
        cache: false,
        panelHeight: 'auto',//自动高度适合
		...
		onLoadSuccess : function(data) {
			var showss=false;
			for (var i = 0; i < data.length; i++) {
				if (data[i].id == is_showsealtype) {
					$('#is_showseal').combobox('setValue', data[i].id);
					showss=true;
					break;
				}
			}
			if(showss){
				$('#is_showseal').combobox('setValue', data[1].id);
			}
		}
	});
	...
	if(checkinput())	
	{
	 ...
	 $.ajax({
        ...
        data:{
          ...
          remark:$("#remark").val(),
          is_showamount:document.getElementsByName("is_showamount")[0].value,
          is_showseal:document.getElementsByName("is_showseal")[0].value
          },          
          type: "post",//数据发送方式   
...
<legend><strong>体检任务基本信息</strong></legend>
			<div class="user-query">
				...
				<dl>
					<dt>显示金额</dt>
					<dd><select class="easyui-combobox" id="is_showamount" name="is_showamount" style="height:26px;width:140px;">
					</select></dd>
				    <dt>显示印章</dt>
					<dd><select class="easyui-combobox" id="is_showseal" name="is_showseal" style="height:26px;width:140px;">
					</select></dd>
					<dt>体检结束日期</dt>
					<dd><input  class="easyui-datebox"   type=text id="exam_date_end" value="<s:property value="model.exam_date_end"/>" style="width:140px;height:26px;"></input></dd>
				</dl>
				<dl>
					<dt>发票抬头</dt>
...
<div id="search-buttons"  class="dialog-button-box">
	<div class="inner-button-box">
	    ...
	    <a href="javascript:void(0)" class="easyui-linkbutton" style="width:80px;" onclick="javascript:$('#dlg-edit-batch').dialog('close')">关闭</a>
	</div>
</div>
