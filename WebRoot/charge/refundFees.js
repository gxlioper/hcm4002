...
function getcustomerInfo(){
	var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
	    $("body").prepend(str);
	$.ajax({
		url:'getCustomerInfoCharge.action?exam_num='+$("#ser_num").val(),
		type:'post',
		success:function(data){
			$(".loading_div").remove();
			if(data == 'null'){
...
function getykfpListGrid(){//已开发票项目列表
	...
	$("#ykfpList").datagrid({
		url: 'getykfpItemListCharge.action',
		queryParams: model,
		rownumbers:true,
		...
		columns:[[{align:'center',field:'ykfp_ck',checkbox:true},
		          ...
		          {align:"center",field:"daily_status","title":"是否日结","width":15,hidden:'true'},
...
function getyksjListGrid(){//查询已收费项目列表
	var model = {"exam_num":$("#ser_num").val()};
	$("#yksjList").datagrid({
		url: 'getyksjItemListCharge.action',
		queryParams: model,
		rownumbers:true,
		pageSize: 15,//每页显示的记录条数，默认为10 
		pageList:[15,30,45,60,75],//可以设置每页记录条数的列表 
...
function tuifei_show(type){
	...
	$("#dlg-tuifei").dialog({
		title:'退费',
		href:'showCollectFeesRefuncdCharge.action?isPrintRecepit='+isPrintRecepit+'&req_nums='+req_nums.toString()+'&peis_trade_code='+req_num,
		buttons:[{text:'关闭',
				  width :80,
				  handler:function(){
...
function chexiaoshoufei(type){
	...
	$.messager.confirm('提示信息','确定撤销所选收费记录吗？',function(r){
		if(r){
			...
			$("body").prepend(str);
			$.ajax({
		        url:'chexiaoshoufeiCharge.action',  
		        data:{
		          exam_id:$("#exam_id").val(),
...
//			}
	...
	for(var i=0;i<data.length;i++){
		if(data[i].exam_status != 'Y' && data[i].exam_status != 'C'&&data[i].is_application!= 'N'){
...
//			alert(1);
	...
	$("body").prepend(str);
	$.ajax({
		url : 'delLisAndPacsApplicationCharge.action',
...
function jiaofeishengqi(){
	...
	$("body").prepend(str);
	$.ajax({
		url : 'paymentApplicationCharge.action',
...
function fapiao_show(){	
	...
	$("#dlg-fapiao").dialog({
		title:'打印发票',
		href:'rechargingSingleInvoickShowCharge.action?bill_type=2',
	});
...
function shouju_show(){	
	...
	$("#dlg-shouju").dialog({
		title:'打印发票',
		href:'rechargingSingleInvoickShowCharge.action?bill_type=1',
	});
...
function zuofeifapiao(){
	...
	$.messager.confirm('提示信息','确定作废所开票据吗？票据号为'+invoice_num.toString(),function(r){
		if(r){
			var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
	   	    $("body").prepend(str);
			$.ajax({
				url : 'invalidInvoiceGeCharge.action',
			        					...
			        					account_num : text.split("-")[1]
			        						...
			        						success : function(text) {
			        							if ((text!='error')&&(text.split("#")[5]=='2')) {
			        								...
			        								FpCancelInv(zuofeifapiaodata,function(objvalue) {
...
//签到发送
function citySignInSend(inter_class,type){
	 insurance_send('dbgj',inter_class,'9100','{"inter_class":"'+inter_class+'","trade_code":"9100","operator_id":"'+$("#userid").val()+'","operator_code":"'+$("#work_other_num").val()+'","operator_name":"'+$("#name").val()+'"}',function(objvalue) {
		 $(".loading_div").remove(); 
		 	var iontername="未知医保";
			if(inter_class=='02'){
				iontername="市医保";
			...
			if(objvalue.error_flag=="1"){
				...
				if(obj.erro_code=="0"){
					$(".loading_div").remove();
					if(type=="1"){
						alert_autoClose("操作提示", iontername+"签到成功！","");
					}
				}else{
					$(".loading_div").remove();
					$.messager.alert("操作提示", iontername+'签到失败',"error");
				}
			}else{
				$(".loading_div").remove();
				$.messager.alert("操作提示", iontername+'签到失败',"error");
