...
function getCityInsuranceItem(){
	$('#city_item').datagrid({    
	    url:'queryDicProvInsuranceClinicItemCharge.action',
	    queryParams:{
	    	item_name:$('#city_item_name').val(),
	    	item_code:$('#city_item_code').val()
	    },
	    pageSize:10,
	    ...
	    pagination:true,
	    columns:[[    
	        {field:'item_code',title:'诊疗项目编码',width:50},    
	...
	var lastIndex;
 	$('#clinicedListCity').datagrid({
	     ...
	     columns:[[    
	        ...
	        {field:'price',title:'项目单价',width:20/* ,editor:{type:'numberbox',options:{precision:2}} */}   
	     ]],
	     onClickCell:function(index, field, row){
				...
				/* var editors1 = $('#clinicedListCity').datagrid('getEditor',{index:index,field:'price'});
...
function clinicCityItem(index){
	...
	if(f == '0'){
		$("#clinicedListCity").datagrid('appendRow',{
			item_code:cityItem.item_code,
			item_name:cityItem.item_name,
			item_num:1,
...
function saveCityClinic(){
	...
	for(var i=0;i<rows.length;i++){
		clinicList.push({
			...
			"medical_item_code":rows[i].item_code,
