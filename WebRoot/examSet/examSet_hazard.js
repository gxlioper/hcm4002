...
function yx_zywhys_html(){
    //已选危害因素
    var t = hazard
    var row_html = '<table><tr><td><div style="float: left ;margin-right:390px">危害因素</div>' +
...
function examhazardList() {
    $.ajax({
        ...
        data:{rows:99999,set_id:$('#zyb_set_id').val()},
        type:'post',
        dataType:'json',
        async:false,
...
function hazaard_save() {
    ...
    for(var i = 0 ; i < hazard.length ; i++){
        //if(hazard[i].status != 1){
            ...
            obj.hazard_year = hazard[i].hazard_year
            obj.set_id = $('#zyb_set_id').val()
            hazard_list.push(obj)
        //}
    }
    if($('#c_hazaard_year').val()=='' && hazard_list.length>0){
        layer.open({
            title: '提示信息'
            ,content: '危害年限不能为空'
        })
    ...
    $.ajax({
        url:'saveExamSetHazard.action',
        type:'post',
        data:{
            hazard_list:JSON.stringify(hazard_list),
            set_id:$('#zyb_set_id').val()
        },
        success:function (text) {
            if(text.split("-")[0]=="ok"){
