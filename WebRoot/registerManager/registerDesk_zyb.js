	 ...
	 var chk_value ="";    
	  ...
	  if(chk_value.length>1){
		 ...
		 var model={
				 ...
				 "employeeID":'',
				 "app_type":document.getElementsByName("app_type")[0].value,	
		 ...
		 url:'getDjtExamInfoUserListall.action',
		 dataType: 'json',
		 queryParams:model,
		 toolbar:'#toolbar',
		 ...
		 columns:[[
		    ...
		    {align:'center',field:'exam_types',title:'体检类型',width:18},	
		 	...
		 	{align:'center',field:'apptypename',title:'类型',width:18}
		 	]],	    	
			...
			rowStyler:function(index,row){
		        ...
		        if(row.apptype=='1') {
		        	...
		        	if (row.freeze==1 && freeze_color){
			        	return 'color:'+freeze_color+';font-weight:bold;';    
			        }
				  	for (var i in customer_type_special.split(",")) {
				  		var customer_type_id = customer_type_special.split(",")[i];
				  		if(row.customer_type_id==customer_type_id && customer_type_special_color){
				  			return 'color:'+customer_type_special_color+';font-weight:bold;';
				  		}
				  	}
...
//    	    });
		...
		iconCls:'icon-add',
	    handler:function(){
	  	  window.parent.addPanel_other("个人职业病登记","getZybDjtRegisterGList.action?exam_id=0","","1");
		...
		iconCls:'icon-save',
	    handler:function(){
	    	window.parent.addPanel_other('团体职业病登记','getZybDjtRegisterTList.action?exam_id=0','"+wx.getIcon_url()+"','1');
	    }
	},{
		text:'预约改期',
		width:90,
  ...
  */
  function f_showexam(val,row){
	  return '<a href=\"javascript:f_examoneshow(\''+row.id+'\',\''+row.exam_type+'\',\''+row.apptype+'\')\">'+row.exam_num+'</a>';
  } 
   ...
   */
  var apptypeflag;
   function f_examoneshow(userid, examtype, apptypeflag) {
	if (apptypeflag == '2') {
		if (examtype == 'G') {
			window.parent.addPanel_other("职业病个人登记",
					"getZybDjtRegisterGList.action?exam_id=" + userid + "", "",
					"1");
		}
		if (examtype == 'T') {
			window.parent.addPanel_other("职业病团体登记",
					"getZybDjtRegisterTList.action?exam_id=" + userid + "", "",
					"1");
		}
	} else {
		if (examtype == 'G') {
			window.parent.addPanel_other("体检个人登记",
					"getDjtRegisterGList.action?exam_id=" + userid + "", "",
					"1");
		}
		if (examtype == 'T') {
			window.parent.addPanel_other("体检团体登记",
					"getDjtRegisterTList.action?exam_id=" + userid + "", "",
					"1");
		}
	}
}
