...
$(document).ready(function() {
	...
	f_findcustomerone($("#oldexam_num").val(),1,1);
	   $("#birthday").datebox({
...
function fnewbutton(vcontralcustflag){
	...
	if((contralcustflag==0)&&(Number($("#exam_id").val())>0)){
...
//	        	$.messager.alert("操作提示", "读取身份证失败", "error");	
				...
				data : {
						...
						success : function(text) {
							...
							if (text.split("-")[0] == 'ok') {
								...
								f_findcustomerone(text.split("-")[1],1,0);
								$('#exam_num').textbox('textbox').focus(); 
								$("#exam_num").textbox('textbox').select();
							} else {
					...
					data : {
						...
						success : function(text) {
								...
								if (Number(text.split("-")[0])>0) {
									f_findcustomeroneidnum(text.split("-")[1],1,0);
								} else {
									$.messager.alert("操作提示","无效查询条件", "error");
								}
							},
	...
	var brushtatle=0;
	var countctr=0;
	function f_findcustomerone(selectexam_id,countctr,brushtatle){
		$.post("getDjtExamOneShow.action", 
		{
			"exam_num" : selectexam_id
		}, function(jsonPost) {
			var customer = eval(jsonPost);			
			setCustomer(customer,countctr,brushtatle);
		}, "json");
	}
	...
	function setCustomer(cumtomersd,countctr,brushtatle)
	{
		if(cumtomersd.exam_type=='T'){
			 $.messager.alert("操作提示","团检人员不能在此页编辑", "error");
		}else{		
	...
	function clearcustomer(){
		...
		$("#mc_no").val('');
		$("#patient_id").val('');
		$("#visit_date").textbox('setValue','');
		$("#visit_no").textbox('setValue','');
...
//					return;
//				}
//				 var url='getHealthCardChargePage.action?exam_num='+$("#exam_num").val()+'&exam_id='+$("#exam_id").val();
//				   newwin = window.open("", "健康卡缴费", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
//				   newwin.moveTo(0,0);
...
//		}
	...
	function djtaddcusItem(){
		...
		//alert($('#exam_num1').val())
		if($('#exam_num1').val()){
 	    	var url='djtcustomeritemaddshow.action?exam_num='+$('#exam_num1').val()+'&sex='+scustsex+'';
