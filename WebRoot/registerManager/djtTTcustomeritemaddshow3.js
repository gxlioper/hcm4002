	...
	if (!isFloat(disvar)) {
		...
		if (!rows.length == 0) {
			...
			for (var j =0 ; j < rows.length-countitemrow; j++){
				var row = rows[j];
				//判断是否为个人 还是团体的  折扣率 大于项目的标准折扣
				if(Number(disvar)>row.item_discount){
					var input_type=$('#exam_indicatorss' + row.item_code).val();
					if(input_type=="T"){
						$('#team_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number(row.itemnum) * Number(disvar) / 10, 2));	
						$('#personal_pay' + row.item_code).val(0);
					}else if(input_type=="G"){
						$('#team_pay' + row.item_code).val(0);
						$('#personal_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number(row.itemnum) * Number(disvar) / 10, 2));	
					}else{
						//混合付费  转为团体的 计算折扣率 为下面方便计算
					...
					var input_type=$('#exam_indicatorss' + row.item_code).val();
					if(input_type=="T"){
						$('#team_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number(row.itemnum) * Number(row.item_discount) / 10, 2));
						$('#personal_pay' + row.item_code).val(0);
					}else if(input_type=="G"){
						$('#team_pay' + row.item_code).val(0);
						$('#personal_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number(row.itemnum) * Number(row.item_discount) / 10, 2));
					}else{
						//混合付费  转为团体的 计算折扣率 为下面方便计算
						is_tuan_ge[j].options[1].selected = true;
						$('#team_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number(row.itemnum) * Number(row.item_discount) / 10, 2));
					...
					$('#discount_input' + row.item_code).val(row.item_discount);
				}
			}
		}
		//计算总金额不超过限额
...
function disamout(disvar) {
	if (!isFloat(disvar)) {
		...
		//计算折扣后金额 不能小于添加的金额
		var rows = $('#selectchangitemlist').datagrid('getRows');
		var addCountMon = 0;//新添加的总金额
		var addDiscountAmount = 0; //新添加项目的最大优惠额度
		var alreadCountMon = 0; //已添加项目的总金额
		var tamtPay = 0  //已添加项目团体付费
		var gamtPay = 0; //已添加项目个人付费
		...
		if (!rows.length == 0) {
			for (var j =0; j < rows.length-countitemrow; j++)//已选择
			{
				...
				if($('#web_Resource').val()=="" || $('#web_Resource').val()==undefined){
					...
					if(Number(row.item_discount)<Number($('#web_Resource').val())){
						addDiscountAmount+=Number(row.item_amount)*((10-Number($('#web_Resource').val()))/10);
					}else{
						addDiscountAmount+=Number(row.item_amount)*((10-Number(row.item_discount))/10);
					}
				}
			...
			{
				tamtPay = decimal(tamtPay + Number(rows[w].team_pay)*Number(rows[w].itemnum),2);
				gamtPay = decimal(gamtPay + Number(rows[w].personal_pay)*Number(rows[w].itemnum),2);
				alreadCountMon+=Number(rows[w].item_amount);
				
			}//j End 
		}
		//alert("==新添加总共能优惠==="+addDiscountAmount+ "==="+ alreadCountMon);
		var amountCha = decimal(Number(gamtPay+tamtPay+addCountMon-disvar),2);
		
		//alert("--新添加已优惠金额--"+amountCha +"==已选择总金额="+addCountMon);
		
		if(decimal(Number(amountCha),2)>decimal(Number(addDiscountAmount),2)){
		    for (var j =0; j < rows.length-countitemrow; j++){
				var row = rows[j];
				row.discount = 10;
			...
			$.messager.alert("提示信息","该添加项目最多优惠到"+decimal(Number(gamtPay+tamtPay+addCountMon-addDiscountAmount),2)+"元","error");
			var amountOld = 0;
			var team_allAmount = 0; //团体结算金额
			var per_allAmount = 0; //个人结算金额
			//计算前折扣金额
			for (var m =0; m < rows.length; m++){
			    var row = rows[m];
				amountOld  = decimal(amountOld + Number(row.personal_pay)+Number(row.team_pay),2);
				team_allAmount = decimal(team_allAmount+Number(row.team_pay),2);
				per_allAmount = decimal(per_allAmount+Number(row.personal_pay),2);
			}
			$("#amount").val(amountOld);
			$("#team_pay").val(team_allAmount)
			$("#personal_pay").val(per_allAmount)
			//计算总金额不超过限额
			calculateMaximumAmount();
		...
		if(amountCha>decimal(Number(addCountMon),2)){
			for (var j =0; j < rows.length-countitemrow; j++){
				//个人分项金额
				var row = rows[j];
				row.discount = 10;
		...
		if (!rows.length == 0) {
			...
			for (var j =0; j < rows.length-countitemrow; j++){
				...
				//判断是团体付费  还是个人付费
				var is_tuan_ge=document.getElementsByName("indicators_input");
				//alert("==新添加总共能优惠==="+addDiscountAmount+"--新添加已优惠金额--"+amountCha);
				//判断是团体结算  个人结算
				if("T"==is_tuan_ge[j].value){
					if(Number(row.item_discount)>Number($('#web_Resource').val())){
						row.team_pay = decimal(Number(row.item_amount)-Number(row.item_amount)*(10-Number(row.item_discount))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					}else{
						row.team_pay = decimal(Number(row.item_amount)-Number(row.item_amount)*(10-Number($('#web_Resource').val()))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					}
					row.itemnum = itemnum_input_is[j].value;
					...
					row.personal_pay = 0;
					is_pay_type = "T";
					if(Number(row.team_pay)==0){
						row.discount = 10;
					}else{
						row.discount = decimal(row.team_pay/Number(row.item_amount)*10, 4);
					}
				}else if("G"==is_tuan_ge[j].value){
					if(Number(row.item_discount)>Number($('#web_Resource').val())){
						row.personal_pay = decimal(Number(row.item_amount)-Number(row.item_amount)*(10-Number(row.item_discount))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					}else{
						row.personal_pay = decimal(Number(row.item_amount)-Number(row.item_amount)*(10-Number($('#web_Resource').val()))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					}
					row.itemnum = itemnum_input_is[j].value;
					row.team_pay = 0;
					...
					is_pay_type = "G";
					if(Number(row.personal_pay)==0){
						row.discount = 10;
					}else{
						row.discount = decimal(row.personal_pay/Number(row.item_amount)*10, 4);
					}
				}else{
					//混合付费 根据金额计算 修改折扣 
					is_tuan_ge[j].options[1].selected = true;
			...
			//alert("==个=="+tamtPay+"==团=="+gamtPay);
			countamt3(); //计算个人金额和团体金额
		...
		if (sexflag) {			
			if(itemtypeflag){
				...
				if($('#web_Resource').val()=="" || $('#web_Resource').val()==undefined ){
					...
					//项目折扣 大于 权限折扣
					if(Number(row.item_discount)>=Number($('#web_Resource').val()) 
							&& Number(row.item_discount)>=Number($("#discount").val())){
						row.discount = row.item_discount;
					}else if(Number(row.item_discount)<=Number($('#web_Resource').val())
							&& Number($("#discount").val())<=Number($('#web_Resource').val())){
						row.discount = $('#web_Resource').val();
					}else if(Number(row.item_discount)<=Number($("#discount").val())
							&& Number($('#web_Resource').val())<=Number($("#discount").val())){
						row.discount = $("#discount").val();
					}
				}
				row.amount = decimal(Number(row.discount)*Number(row.amount)/10,2);
...
function djtcustadd() {
	...
	if (itemrows.length > 0) {
		//判断 是否有最大金额
		if($("#maximumAmount").val()!=100000000){
			//证明有最大金额
			if(Number($("#team_pay").val())>Number($("#maximumAmount").val())){
				$.messager.alert("操作提示", "团体金额不能大于"+Number($("#maximumAmount").val())+"元", "error");
				return;
			}
		}
	...
	$("#changitemlist").datagrid(
			{
				...
				onDblClickRow : function(index, row) {
					...
					if (!isFloat($("#discount").val())) {
						...
						if($('#web_Resource').val()=="" || $('#web_Resource').val()==undefined ){
							row.discount = '10';
							row.amount = decimal(row.item_amount*row.itemnum, 2);
						}else{
							//项目折扣 大于 权限折扣
							if(Number(row.item_discount)>=Number($('#web_Resource').val()) 
									&& Number(row.item_discount)>=Number($("#discount").val())){
								...
								row.discount = $('#web_Resource').val();
								row.amount = decimal(row.item_amount*row.itemnum
										* Number($('#web_Resource').val()) / 10, 2);
							}else if(Number(row.item_discount)<=Number($("#discount").val())
									&& Number($('#web_Resource').val())<=Number($("#discount").val())){
								row.discount = $("#discount").val();
								row.amount = decimal(row.item_amount*row.itemnum
										* Number($("#discount").val()) / 10, 2);
							}
						}
...
function setfzchareitemListGrid() {
	...
	$("#selectchangitemlist").datagrid({
		...
		columns : [ [ 
		    ...
		    {align : 'center',field : 'item_discount',title : '项目折扣',width : 16,hidden:true}, 
...
function f_discount_input(row_item_code, row_item_amt, discount_input) {
	...
	exam_indicators_input=$('#exam_indicatorss' + row_item_code).val(); // 类别
	if (!isSZZoo(itemnumValue)){
		 itemnumValue=1;
	}
	 ...
	 var rows = $('#selectchangitemlist').datagrid('getRows');
	 if (!rows.length == 0) {
		for (var j = 0; j <= rows.length-countitemrow-1; j++){
			var row = rows[j];
			if(row_item_code==row.item_code){
	 ...
	 if(Number(itemDiscount) > discountValue){
		 $.messager.alert("提示信息","该项目最小折扣权限为"+itemDiscount+"折","error");
		 $('#discount_input'+row_item_code).val(Number(itemDiscount));
		 discountValue=Number(itemDiscount);
		 //判断付费类别
		 if(exam_indicators_input=="T"){
			 $('#team_pay' + row_item_code).val(decimal(Number(itemDiscount)*Number(invMoneyValue)/10,2));
			 $('#personal_pay' + row_item_code).val(0);
		 }else if(exam_indicators_input=='G'){
			 $('#team_pay' + row_item_code).val(0);
			 $('#personal_pay' + row_item_code).val(decimal(Number(itemDiscount)*Number(invMoneyValue)/10,2));
		 }else{
			 $('#team_pay' + row_item_code).val(decimal(Number(itemDiscount)*Number(invMoneyValue)/10,2));
			 $('#personal_pay' + row_item_code).val(0);
			 //计算总金额不超过限额
	...
	if(Number(discountValue)<Number($('#web_Resource').val()) && Number(discountValue)!=Number(xxzk)){
...
function f_team_pay_input(row_item_code, row_item_amt, team_pay_input) {
	...
	if (isNaN(Number(team_pay_input))) {  //输入的不为数字
		var depositValue = decimal(Number(invMoneyValue)*Number(itemnumValue)
				* Number(discountValue) / 10, 2); //折扣率
		if(exam_indicators_input=='T'){
			$('#team_pay' + row_item_code).val(depositValue);
			$('#personal_pay' + row_item_code).val(0);
		}else if(exam_indicators_input=='G'){
		...
		var zhekoulvqx =10; //操作员最大折扣
		if( $('#web_Resource').val()!=undefined && $('#web_Resource').val()!=""){
			zhekoulvqx=decimal(Number($('#web_Resource').val()), 4);
		}
		//折扣率
		var depositValue = decimal(Number(invMoneyValue)*Number(itemnumValue)* Number(discountValue) / 10, 4);
		
		var itemDiscount; //单选项目的最大折扣
		 var rows = $('#selectchangitemlist').datagrid('getRows');
		 if (!rows.length == 0) {
			for (var j = 0; j <= rows.length-countitemrow-1; j++){
				var row = rows[j];
		...
		if (Number(team_pay_input) > depositValue) { //新输入的 大于以前的
			if(exam_indicators_input=='T'){		
				...
				if((zhekoulvqx>dzsize)||(dzsize>10)){
					...
					if(Number(itemDiscount)>zhekoulvqx){
						$.messager.alert("提示信息","最大打折金额为"+decimal(Number(invMoneyValue)*Number(itemDiscount)/10,2)+"元","error");
					}else{
						$.messager.alert("提示信息","最大打折金额为"+decimal(Number(invMoneyValue)*Number(zhekoulvqx)/10,2)+"元","error");
					}
				}else{
					$('#personal_pay' + row_item_code).val(0);
			...
			if(exam_indicators_input=='T'){
				//输入后新折扣
				var dzsize = decimal(Number(team_pay_input)/Number(invMoneyValue)*Number(itemnumValue) * 10, 4);
				if((zhekoulvqx>dzsize)||(dzsize>10)){ //判断资源是否大于输入的
					$('#team_pay' + row_item_code).val(decimal(Number(invMoneyValue)*Number(itemnumValue), 2));
					$('#personal_pay' + row_item_code).val(0);
					$('#discount_input' + row_item_code).val(10);
					if(Number(itemDiscount)>zhekoulvqx){
					...
					if(Number(itemDiscount) > dzsize){
						 $.messager.alert("提示信息","该项目最小折扣权限为"+itemDiscount+"折","error");
						 $('#discount_input' + row_item_code).val(itemDiscount);
						 $('#team_pay' + row_item_code).val(decimal(Number(invMoneyValue)*Number(itemnumValue)*Number(itemDiscount)/10, 2));
						 return;
					}else{
						$('#discount_input' + row_item_code).val(dzsize);
					}
...
//查询该分组的最大限制金额
function queryGroupInfoAmount(){
	if($("#group_id").val()!=0 && $("#group_id").val()!=null && $("#group_id").val()!=""){
		$.ajax({
			url : 'queryGroupInfoAmount.action',
			data : {group_id : $("#group_id").val()},
			type : "post",// 数据发送方式
			success : function(text) {
				if (text.split("-")[0] == 'ok') {
					//最大金额无限制
					if("NoAmount"==text.split("-")[1]){
						$("#maximumAmount").val(100000000);//不限制最大金额
					}else{
						$("#maximumAmount").val(text.split("-")[1]);
					}
				} else {
					$.messager.alert("操作提示", text.split("-")[1], "error");
				}
...
//超过限制金额的自动计算为个人收费
function calculateMaximumAmount(){
	if($("#maximumAmount").val()==100000000){
		return;
	}
	var addPerTeamAmount = 0; //新添加项目团体付费
	var addPerPayAmount = 0; //新添加项目个人付费
	var teamPayAmount = 0;  //已添加项目团体付费
	var perPayAmount = 0;  //已添加项目个人付费
	var rows = $('#selectchangitemlist').datagrid('getRows');
	var is_tuan_ge=document.getElementsByName("indicators_input");
	
	for (var a =0; a < rows.length-countitemrow; a++){
		if(is_tuan_ge[a].value=="G"){//修改 个人付费为团体付费
			is_tuan_ge[a].options[1].selected = true;
			$("#team_pay"+rows[a].item_code).val($("#personal_pay"+rows[a].item_code).val()); ////团体输入框金额
			$("#personal_pay"+rows[a].item_code).val(0);
		}else if(is_tuan_ge[a].value=="GT"){
			is_tuan_ge[a].options[1].selected = true;
			$("#team_pay"+rows[a].item_code).val(Number($("#team_pay"+rows[a].item_code).val())+Number($("#personal_pay"+rows[a].item_code).val())); ////团体输入框金额
			$("#personal_pay"+rows[a].item_code).val(0);
		}
	}
	//先判断已添加的项目团体结算金额
	...
	if (!rows.length == 0) {
		//新添加
		for (var j =0; j < rows.length-countitemrow; j++){
			addPerTeamAmount = decimal(addPerTeamAmount + Number(team_pay_inputss[j].value)*Number(itemnum_input_is[j].value),2);
			addPerPayAmount = decimal(addPerPayAmount + Number(personal_pay_inputss[j].value)*Number(itemnum_input_is[j].value),2);
		}
		//已经添加的
		for (var w = rows.length-countitemrow; w < rows.length; w++){
			teamPayAmount = decimal(teamPayAmount + Number(rows[w].team_pay)*Number(rows[w].itemnum),2);
			perPayAmount = decimal(perPayAmount + Number(rows[w].personal_pay)*Number(rows[w].itemnum),2);
		}
	}
	//alert("--新加个人付费--"+addPerPayAmount+"--新加团体付费--"+addPerTeamAmount+"--团体付费--"+teamPayAmount+"--个人付费--"+perPayAmount);
	//已添加的项目金额小于最大额度 
	var standardAmount = teamPayAmount;
	if(Number(teamPayAmount)<Number($("#maximumAmount").val())){
		//循环计算新添加项目
		if(rows.length-countitemrow>0){
			//alert(rows.length-countitemrow);
			for (var q =rows.length-countitemrow-1; q >=0 ; q--){
				standardAmount +=  decimal(Number(team_pay_inputss[q].value)*Number(itemnum_input_is[q].value),2);
				//alert(standardAmount);
				if(decimal(standardAmount,2)<Number($("#maximumAmount").val())){
					//标准金额等于限定金额  团体付费
					is_tuan_ge[q].options[1].selected = true;
					if(Number($("#team_pay"+rows[q].item_code).val())==0 || is_tuan_ge[q].value=="G"){
						$("#team_pay"+rows[q].item_code).val($("#personal_pay"+rows[q].item_code).val()); ////团体输入框金额
						$("#personal_pay"+rows[q].item_code).val(0);
					}else if(is_tuan_ge[q].value=="GT"){ //以前为混合付费
						$("#team_pay"+rows[q].item_code).val(Number($("#personal_pay"+rows[q].item_code).val())+Number($("#team_pay"+rows[q].item_code).val())); ////团体输入框金额
						$("#personal_pay"+rows[q].item_code).val(0);
					}
				}else if(decimal(standardAmount,2)>Number($("#maximumAmount").val())){
					// 情况一 此项>500  再减一项仍然>500
					if(Number(decimal(standardAmount,2)-Number(team_pay_inputss[q].value)*Number(itemnum_input_is[q].value))>=Number($("#maximumAmount").val())){
						//此项为个人付费
						is_tuan_ge[q].options[0].selected = true;
						$("#personal_pay"+rows[q].item_code).val($("#team_pay"+rows[q].item_code).val());
						$("#team_pay"+rows[q].item_code).val(0); ////团体输入框金额
					}else if(Number(standardAmount-Number(team_pay_inputss[q].value)*Number(itemnum_input_is[q].value))<Number($("#maximumAmount").val())){
						// 情况二 此项>500  再减一项仍然<500     此项为混合付费 
						is_tuan_ge[q].options[2].selected = true;
						//alert("==现在金额="+decimal(standardAmount,2));
						//alert("==第一个项目=="+Number(team_pay_inputss[0].value)*Number(itemnum_input_is[0].value)+"==已添加项目团体付费=="+teamPayAmount+"===新添加的团体付费=="+addPerTeamAmount);
						var tuan = decimal(Number(team_pay_inputss[q].value)*Number(itemnum_input_is[q].value)-(standardAmount-Number($("#maximumAmount").val())),2);
						var per = decimal(standardAmount-Number($("#maximumAmount").val()),2);
						//alert("---团体费用-"+tuan+"--个人费用--"+per);
						if(tuan>0){
							$("#team_pay"+rows[q].item_code).val(decimal(tuan,2));
							$("#personal_pay"+rows[q].item_code).val(per);
						}else{
							//进行折扣时候 小于零
							$("#team_pay"+rows[q].item_code).val(decimal(Number(team_pay_inputss[q].value)*Number(itemnum_input_is[q].value)+tuan,2));
							$("#personal_pay"+rows[q].item_code).val(Math.abs(tuan));
						}
					}/*else if(standardAmount==Number($("#maximumAmount").val())){
						//标准金额等于限定金额  个人付费
						is_tuan_ge[q].options[0].selected = true;
						$("#personal_pay"+rows[q].item_code).val($("#team_pay"+rows[q].item_code).val());
						$("#team_pay"+rows[q].item_code).val(0); ////团体输入框金额
