...
/*//动态设置列表折扣率、项目折扣后金额、折扣后总额
function check(str){
	var datas=$('#selectchangitemlist').datagrid('getData');
	var amount=0;
    if (!datas == "") {
	    for (var j = 0; j <= datas.rows.length - 1; j++)// 已选择
	    {
			var row = datas.rows[j];
			row.discount = str;
			$('#selectchangitemlist').datagrid('updateRow',{index:j,row:row.discount});
			row.amount = decimal(Number(row.item_amount)*Number(row.itemnum) * Number(str) / 10, 2);	
		...
		if (!datas == "") {
			for (var j = 0; j <= datas.rows.length-1; j++)//已选择
			{
				...
				if(Number(disvar)<Number(xmdiscount)){
					...
					row.amount = decimal(Number(row.item_amount)*Number(row.itemnum) * Number(xmdiscount) / 10, 2);
				}else{
					row.discount = disvar;
...
function disamout(disvar) {
	if (!isFloat(disvar)) {
		...
		if(web_Resource=="" || web_Resource==undefined){
			...
			var amountOld = 0;
			//计算前折扣金额
			for (var m =0; m < datas.rows.length; m++){
				    var row = datas.rows[m];
		...
		if (!datas.rows.length == 0) {
			...
			for (var j =0; j <= datas.rows.length-1; j++){
				var row = datas.rows[j];
				var xmdiscount=row.item_discount;//收费项目表折扣率				
				if(xmdiscount>newdiscont){
					$.messager.alert("操作提示", ""+row.item_name+"--折扣率小于收费项目折扣率，要按照收费项目折扣率执行！", "");
					//所有项目的总额
					row.discount = xmdiscount;
					row.amount = decimal(Number(row.item_amount)* Number(xmdiscount) / 10, 2);
				}else{
					//所有项目的总额
					row.discount = newdiscont;
					row.amount = decimal(Number(row.item_amount)* Number(newdiscont) / 10, 2);
				}
				$('#selectchangitemlist').datagrid('refreshRow', j);
			}
		}
...
function countamt3() {
	...
	var newdiscount=decimal(newamt * 10 / Number(oldamt),4);
	$("#item_amount").val(oldamt);
	$("#amount").val(newamt);
	...
	function djtcustadd() {
		...
		if (itemrows.length>0) {
			...
			$.ajax({
				url : 'djtGcustSaveDiscount.action',
				data : {
							exam_num : $("#exam_num").val(),
...
/*	*//**
    ...
    function calcMoneyChange(desindex){
      ...
      // alert("折扣1"+depositValue+"折扣66"+$('#web_Resource').val()+"金额1"+amountValue+"金额2"+x_jine);
        ...
        if(depositValue<web_Resource && amountValue != x_jine && depositValue!=0){
        	$.messager.alert("提示信息","本操作员最大权限只能打"+web_Resource+"折！","error");
    ...
    function calcDeposit(desindex){
        ...
        var itemnumValue=rows[desindex].itemnum; 
        x_discount = rows[desindex].item_discount;//获取项目折扣率
        if (!isSZZoo(itemnumValue)){
   		 itemnumValue=1;
   	    }
        var invMoneyValue = decimal(rows[desindex].item_amount*Number(itemnumValue),2);           // 金额值
        ...
        if(discountValue<web_Resource){
        	$.messager.alert("提示信息","本操作员最大权限只能打"+web_Resource+"折！","error");
        	//rows[desindex].discount=x_zhekou;
        	return;
        }
		if(Number(discountValue)<Number(x_discount)){
			$.messager.alert("操作提示", ""+rows[desindex].item_name+"--折扣率小于收费项目折扣率，要按照收费项目折扣率执行！", "");
			rows[desindex].discount=x_discount; 
            rows[desindex].amount=decimal(Number(rows[desindex].item_amount)*Number(itemnumValue) * Number(x_discount) /10, 2);    // 折扣额  赋值   
		}else{
		...
		$("#selectchangitemlist").datagrid({
			...
			columns : [[ 
			  /* {align : "center",field : "fxfzddd",title : "删除",width : 10,	"formatter" : f_dellchargitem},*/
			   ...
			   {align : 'center',field : 'itemnum',title : '数量',width : 10,"formatter" : f_itemnum,editor:'text'}, 
			   {align : 'center',field : 'discount',title : '折扣率',width : 15,"formatter" : f_discount,editor:'text'}, 
			   {align : 'center',field : 'amount',title : '金额',	width : 20,	"formatter" : f_personal_pay,editor:'text'},
