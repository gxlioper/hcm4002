...
//	            total += rows[i]['item_discount'];
	    ...
	    if(Number($(this).val())<Number($('#webResource').val())){
	    	...
	    	if(Number($(this).val())<total){
	    		$.messager.confirm('提示信息','存在项目折扣小于折扣的项目，是否继续？',function(r){
					if(r){
						updateAlldiscount($("#zongzhekou").val());
...
function getwitemGrid(){//查询未收费项目列表
	 ...
	 columns:[[
	    ...
	    {align:'center',field:'item_code',title:'项目编码',width:12},
	 	...
	 	{align:'center',field:'item_discount',title:'项目最大打折',width:15},
    ...
    onAfterEdit:function(rowIndex, rowData, changes){
    	...
    	if(changes.discount != undefined){ //修改单一折扣
    		 var item_discount = rowData.item_discount;
    		 if(Number(changes.discount)<Number($('#webResource').val())){
    	  	    	$.messager.alert('提示信息',"本操作员最大权限可打"+$('#webResource').val()+"折！",'error');
    	  	    	rowData.discount=olddiscount;
    	  	    	
    	  	 }else if(Number(changes.discount)<item_discount){
    	  		$.messager.alert('提示信息',"本项目最大可打"+item_discount+"折！",'error');
	  	    	rowData.discount=olddiscount;
    	  	 }else if($('#webResource').val()=="" || $('#webResource').val()==undefined){
    	  	    	$.messager.alert('提示信息',"没有打折权限",'error');
    	  	    	rowData.discount=olddiscount;
...
function moling(){
	...
	if(yinshou > shishou){
		...
		rows[i].personal_pay = rows[i].amount;
...
function updateAlldiscount(disvar){
	if (!isFloat(disvar)) {
		...
		if (!rows.length == 0) {
			...
			{
				var row = rows[j];
				if(row.item_discount>=disvar){
					 disvar=row.item_discount;
				}
					row.discount = disvar;
