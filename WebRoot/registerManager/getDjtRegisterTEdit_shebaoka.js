...
function fnewbutton(vcontralcustflag){
	...
	if(Number($("#company_id").val())<=0){
		...
		$.messager.alert("操作提示", "体检任务不能为空", "error");
	}else if((contralcustflag==0)&&(Number($("#exam_id").val())>0)){
		$.messager.alert("操作提示", "记录存在，不能新增", "error");
	}else if((contralcustflag==1)&&(Number($("#exam_id").val())>0)){
		$.messager.alert("操作提示", "记录存在，不能预约", "error");
	}else if((contralcustflag==2)&&(Number($("#exam_id").val())<=0)){
...
function IdCard(UUserCard,num){
	...
	function setCustomer(cumtomersd)
	{
		 if((contralcustflag==2)&&(cumtomersd.exam_type=='G')){
			...
			$("#membership_card").val(cumtomersd.membership_card);   //社保卡号
			...
			if(cumtomersd.examcount.length>10){
				document.getElementById("examcount").innerHTML="您是第"+cumtomersd.examcount;
			}else{
	...
	function djtTTaddcusItem(){
		...
		if(Number($("#exam_id").val())<=0){
 	    	...
 	    	var url='djtTTcustomeritemaddshow.action?exam_id='+$("#exam_id").val()+'&sex='+scustsex+'&exam_indicator='+$("#exam_indicator").val()+'';
 	    	newWindow = window.open(url, "人员团体加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
 	    	newWindow.focus();
 	    	//timer = setInterval("djtupdateAfterClose()", 1000);
 	   	}
	...
	function djtTGaddcusItem(){
		...
		if(Number($("#exam_id").val())<=0){
 	    	...
 	    	var url='djtTGcustomeritemaddshow.action?exam_id='+$("#exam_id").val()+'&sex='+scustsex+'&exam_indicator='+$("#exam_indicator").val()+'';
		  					...
		  					data : {
		  							...
		  							success : function(text) {
		  								...
		  								if (text.split("-")[0] == 'ok') {//一个体检信息
		  									   $("#exam_num").val(text.split("-")[1]);
		  										$.messager.confirm('提示信息','个人缴费 '+geRenMoney+' 元 是否报到？',function(r){
		  										if(r){
...
function new_printdjd(){
	if($("#isNeedGuide").val()=="Y"){
		if($("#rylb").combobox("getText")!=""){
			var rylbname='该人员已打印  人员类别:' +$("#rylb").combobox("getText")+'     是否继续打印';
		}else{
			var rylbname = '该人员已经打印 是否继续打印';
			
		}
		$.messager.confirm('提示信息',rylbname,function(r){
			if(r){
		...
		if($("#rylb").combobox("getText")!=""){
			var rylbname = $("#rylb").combobox("getText");
			$.messager.alert("提示信息","该人员的人员类别:"+rylbname+'   是否继续打印','info');
		}
		if($("#barcode_print_type").val() == '1'){//调用旧打印程序
			printdjd();
		}else if($("#barcode_print_type").val() == '2'){//调用新打印程序
	...
	if($("#exam_num").val().length>0){
		...
		$.ajax({
			...
			success : function(text) {
				if (text.split("-")[0] == 'ok') {
					$(".loading_div").remove();
					var url = 'GuidBarServices://&P&0&'+$("#userid").val()+'&barcode&'+$("#exam_num").val()+'&&'+bar_calss+'&1';
					RunServerExe(url);
...
function duqushebaoka() {
	var sfz_div_code = $("#sfz_div_code").val();
	var data = read_card_sbk(sfz_div_code);
	if (data.error_flag == "0") {
		$('#membership_card').val(data.sbno);
		$("#custname").val(data.name);
		$("#id_num").val(data.certno);
		$("#sex").textbox('setValue',data.sex);
		$("#nation").textbox('setValue',data.nation);
		var bords = data.birth;
		if(bords.length==8){
			var birthday= bords.substring(0,4)+"-"+bords.substring(4,6)+"-"+bords.substring(6,8);
			$('#birthday').datebox('setValue',birthday);						
		...
		$.messager.alert("操作提示", "读取社保卡失败", "error");
	}
}