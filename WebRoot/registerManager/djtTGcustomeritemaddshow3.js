		...
		if (!rows.length == 0) {
			for (var j = 0; j < rows.length-countitemrow; j++)// 已选择
			{
				...
				if(Number(disvar)>row.item_discount){
					...
					$('#personal_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()) * Number(disvar) / 10, 2));	
				}else{
					$('#discount_input' + row.item_code).val(row.item_discount);
					$('#personal_pay' + row.item_code).val(decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()) * Number(row.item_discount) / 10, 2));
				}
			}
		}
		countamt();
...
function disamout(disvar) {
	if (!isFloat(disvar)) {
			...
			for (var j =0; j < rows.length-countitemrow; j++){
				...
				if($('#web_Resource').val()=="" || $('#web_Resource').val()==undefined){
					...
					if(Number(row.item_discount)<Number($('#web_Resource').val())){
						addDiscountAmount+=decimal(Number(row.item_amount)*Number(itemnum_input_is[j].value)*((10-Number($('#web_Resource').val()))/10),2);
					}else{
						addDiscountAmount+=decimal(Number(row.item_amount)*Number(itemnum_input_is[j].value)*((10-Number(row.item_discount))/10),2);
					}
				}
			}   
		...
		//alert("==已有项目总金额=="+alreadCountMon+"===团体付费=="+tamtPay+"===个人付费=="+gamtPay);
		//alert("==已有项目优惠金额==="+decimal(Number(alreadCountMon)-Number(tamtPay)-Number(gamtPay),2));
		//新添加项目优惠的金额
		...
		if(decimal(Number(amountCha),2)>decimal(Number(addDiscountAmount),2)){
		    for (var j =0; j < rows.length-countitemrow; j++){
				var row = rows[j];
				row.itemnum = Number(itemnum_input_is[j].value);
				row.discount = 10;
				row.personal_pay = decimal(Number(row.item_amount)*Number(itemnum_input_is[j].value),2);	
		...
		if (!rows.length == 0) {
			...
			for (var j =0; j < rows.length-countitemrow; j++){
				...
				//判断
				if(Number(row.item_discount)>Number($('#web_Resource').val())){
					row.personal_pay = decimal(Number(row.item_amount)*Number(itemnum_input_is[j].value)-Number(row.item_amount)*Number(itemnum_input_is[j].value)*(10-Number(row.item_discount))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
				}else{
					row.personal_pay = decimal(Number(row.item_amount)*Number(itemnum_input_is[j].value)-Number(row.item_amount)*Number(itemnum_input_is[j].value)*(10-Number($('#web_Resource').val()))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
				}
				row.itemnum = Number(itemnum_input_is[j].value);
				row.discount = decimal(row.personal_pay/(Number(row.item_amount)*Number(itemnum_input_is[j].value))*10,4);
				allPerAmount = decimal(Number(allPerAmount+Number(row.personal_pay)),2);
				$('#selectchangitemlist').datagrid('refreshRow', j);
			}   
			
			//个人结算总额
			$("#personal_pay").val(decimal(allPerAmount+gamtPay,2));
		}
	}
}
...
function countamt3() {
	...
	//计算折扣率
	$("#discount").val(decimal(Number(tamt+gamt) / Number(yuanGamont+itemamout) * 10, 4));
}
/***
 * 项目带出项目
 * @returns {String}
...
function setfzchareitemListGrid() {
	...
	$("#selectchangitemlist").datagrid({
		...
		columns : [ [ 
		          ...
		          {align : 'center',field : 'team_pay',title : '团体付费',width : 15}, 
...
function f_discount_input(row_item_code, row_item_amt,discount_input) {
	...
	//资源
	if(Number(discountValue) < Number($('#web_Resource').val()) && Number(discountValue) != Number(xzhekou)){
		$.messager.alert("提示信息","本操作员最大权限可打"+$('#web_Resource').val()+"折！","error");
		$('#discount_input'+row_item_code).val(xzhekou);
		discountValue=xzhekou;
	}
...
function f_personal_pay_input(row_item_code, row_item_amt,personal_pay_input) {
	...
	if (isNaN(Number(personal_pay_input))) {
		...
		if (Number(personal_pay_input) > invMoneyValue) {	
			...
			if(Number(itemDiscount) > depositValue){
				 $.messager.alert("提示信息","该项目最小折扣权限为"+itemDiscount+"折","error");
