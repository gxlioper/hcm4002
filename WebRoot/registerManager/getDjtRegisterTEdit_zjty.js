...
function fnewbutton(vcontralcustflag){
	...
	}else if((contralcustflag==2)&&($("#isprivateflag").val()=="Y")){
...
function IdCard(UUserCard,num){
	...
	function setCustomer(cumtomersd)
	{
		...
		$('#budget_amount').textbox('setValue', cumtomersd.budget_amount);
	...
	function clearcustomer(){
		...
		$("#picture_Path").val('');
		$('#budget_amount').textbox('setValue', '');
		$("#employeeID").val('');
...
//		    	if(getPatientid()=="ok"){
		    		djtTGaddcusItem();
//		    	} else {
//		    		$.messager.alert("警告信息","体检信息已上传-不能加项","error");
//		    	}
		    }
		},{
			text:'团体加项',
...
//		    	}
	...
	function djtTTaddcusItem(){
		...
		if(Number($("#exam_id").val())<=0){
 	    	...
 	    	var url='djtTTcustomeritemaddshow.action?exam_num='+$('#exam_num').val()+'&exam_id='+$("#exam_id").val()+'&sex='+scustsex+'&exam_indicator='+$("#exam_indicator").val()+'';
 	    	newWindow = window.open(url, "人员团体加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
 	    	newWindow.focus();
 	    	//timer = setInterval("djtupdateAfterClose()", 1000);
 	   	}
	...
	function djtTGaddcusItem(){
		...
		if(Number($("#exam_id").val())<=0){
 	    	...
 	    	var url='djtTGcustomeritemaddshow.action?exam_num='+$('#exam_num').val()+'&exam_id='+$("#exam_id").val()+'&sex='+scustsex+'&exam_indicator='+$("#exam_indicator").val()+'';
 ...
 * +++++++++++++++++++++++++++++++上传体检信息+++++++++++++++++++++++++++++++++++++++++++++++++
 */
function getExaminfoSet(){
	if(document.getElementById("exam_id").value==""||document.getElementById("exam_id").value==0){
		$.messager.alert("警告信息","请选择体检人信息","error");
		return;
	}
	 var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
	 $("body").prepend(str);
	$.ajax({
		url:'getExaminfoSet.action',
...
function getPatientid(){
	if($('#exam_id').val()==""){
		$.messager.alert("警告信息","体检用户无效","error");
		return;
	}
	var fal = "no";
	$.ajax({
		url:"getPatientId.action",
		async: false,
...
function printdy(extitem,pacsitem){
	if(barcode_print_type == '1'){//调用旧打印程序
		printBar();
	}else if(barcode_print_type == '2'){//调用新打印程序
		
		var lisids = new Array();
		var itemcode = new Array();
		
		var lextitemlists = extitem;
	    $.each(lextitemlists, function(index, item){
	    	lisids.push(item.id);
	    	itemcode.push(item.item_code);
	    });
	    var pacsitemlists = pacsitem;
			...
			data : {
				...
				ids : lisids.toString()
					...
					success : function(text) {
						$(".loading_div").remove();
						if($("#exam_num").val()==""){
							$.messager.alert("警告信息","体检编码不能为空","error");
							return;
						}
						$.ajax({
							url : 'updateBarcodePrintStatus.action',
...
//									 var exeurl ='GuidBarServices://&barcode&'+$("#exam_num").val()+'&'+itemcode.toString()+'&3'; //打印档案号
//								 }else{
