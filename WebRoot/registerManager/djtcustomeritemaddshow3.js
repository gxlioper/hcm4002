...
function discount(disvar) {
	if (!isFloat(disvar)) {
		...
		if (!rows.length == 0) {
			...
			{
				var row = rows[j];
				if(disvar>row.item_discount){
					row.discount = disvar;
					row.amount = decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()) * Number(disvar) / 10, 2);	
					row.itemnum = Number($('#itemnum_input' + row.item_code).val());
				}else{
					row.discount = row.item_discount;
					row.amount = decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()) * Number(row.item_discount) / 10, 2);
					row.itemnum = Number($('#itemnum_input' + row.item_code).val());
				}
...
//	alert(disvar);
	if (!isFloat(disvar)) {
		...
		var addCountMon = 0; // 新添加项目的总金额
		var countAllMon = 0; //已添加项目总金额
		var countMonAlready = 0; //已添加项目优惠后金额
		if (!rows.length == 0) {
			//计算已添加项目的最大优惠额度
			for (var j =0; j < rows.length-countitemrow; j++)//已选择
			{
				var row = rows[j];
				addCountMon+=decimal(Number(row.item_amount)*Number($('#itemnum_input'+row.item_code).val()),2);
				if($('#web_Resource').val()=="" || $('#web_Resource').val()==undefined){
					//没有打折权限
					addDiscountAmount=0;
				}else{
					if(Number(row.item_discount)<Number($('#web_Resource').val())){
						addDiscountAmount+=decimal(Number(row.item_amount)*Number($('#itemnum_input'+row.item_code).val())*((10-Number($('#web_Resource').val()))/10),2);
					}else{
						addDiscountAmount+=decimal(Number(row.item_amount)*Number($('#itemnum_input'+row.item_code).val())*((10-Number(row.item_discount))/10),2);
					}
				}
			} 
			//计算已有项目的优惠金额
			...
			{
				var row = rows[k];
				countAllMon+=decimal(Number(row.item_amount)*Number(row.itemnum),2);
				countMonAlready+=Number(row.amount);
			}
		}
		//alert("==新添加项目的最大优惠额度=="+addDiscountAmount+"==新添加项目的总金额=="+addCountMon);
		//alert(countAllMon+"===="+countMonAlready);
		//amountCha 新添加项目 优惠价格
		...
		if(decimal(Number(amountCha),2)>decimal(Number(addDiscountAmount),2)){
		    for (var j =0; j < rows.length-countitemrow; j++){
				...
				row.itemnum = Number($('#itemnum_input' + row.item_code).val());
				row.discount = 10;
				row.amount = decimal(row.item_amount*Number($('#itemnum_input' + row.item_code).val()),2);	
				$('#selectchangitemlist').datagrid('refreshRow', j);
			}
			$.messager.alert("提示信息","该添加项目最多优惠到"+decimal(Number(countMonAlready+addCountMon-addDiscountAmount),2)+"元","error");
			var amountOld = 0;
			//计算前折扣金额
		...
		//新加项目总共能优惠金额
		//alert("==总可能优惠的金额==="+addDiscountAmount+"==优惠的金额=="+amountCha); 
		//添加项目优惠的总金额
		...
		if (!rows.length == 0) {
			...
			var addDiscont = decimal(Number(addAmount-allAmount) / Number(addAmount) * 10, 2);*/
			for (var j =0; j < rows.length-countitemrow; j++){
				var row = rows[j];
				//所有添加项目的总额
				...
				if(Number(row.item_discount)>Number($('#web_Resource').val())){
					...
					row.itemnum = Number($('#itemnum_input' + row.item_code).val());
					row.amount = decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val())-Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val())*(10-Number(row.item_discount))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					row.discount = decimal(row.amount/(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()))*10,4);
				}else{
					row.itemnum = Number($('#itemnum_input' + row.item_code).val());
					row.amount = decimal(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val())-Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val())*(10-Number($('#web_Resource').val()))/10*(Number(amountCha)/Number(addDiscountAmount)),2);
					if(Number(row.item_amount)==0){
						row.discount =10;
					}else{
						row.discount = decimal(row.amount/(Number(row.item_amount)*Number($('#itemnum_input' + row.item_code).val()))*10,4);
					}
		...
		$("#changitemlist").datagrid(
				{
					...
					columns : [[ {align : 'left',	field : 'item_code',title : '项目编码',width : 10},
					         ...
					         {align : 'center',field : 'item_discount',title : '项目最大折扣',width : 18},
					         {align : 'center',field : 'item_amount',title : '金额',width : 14}
					         ]],
					onLoadSuccess : function(value) {
						$("#datatotal").val(value.total);
						$("#itemname").focus();
	...
	function setfzchareitemListGrid() {
		...
		$("#selectchangitemlist").datagrid({
			...
			columns : [[ 
			   ...
			   {align : 'left',field : 'item_code',title : '项目编码',width : 16}, 
			   ...
			   {align : 'center',field : 'item_amount',title : '原金额',width : 13},
			   {align : 'center',field : 'itemnum',title : '数量',width : 10,"formatter" : f_itemnum}, 
			   {align : 'center',field : 'item_discount',title : '项目折扣率',width : 15,hidden:true}, 
		...
		if (isNaN(Number(personal_pay_input))) {
			...
			if (Number(personal_pay_input) > invMoneyValue) {	
				...
				if(Number(itemDiscount) > depositValue){
					 $.messager.alert("提示信息","该项目最小折扣权限为"+itemDiscount+"折","error");
					 $('#personal_pay'+row_item_code).val(xjineFinal);
					 return;
				}
				...
				//资源
				if(depositValue < $('#web_Resource').val() && $('#personal_pay'+row_item_code).val() != xjineFinal){
					$.messager.alert("提示信息","本操作员最大权限可打"+$('#web_Resource').val()+"折！","error");
					$('#personal_pay'+row_item_code).val(xjineFinal);
					return;
				}
