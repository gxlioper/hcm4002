	...
	//补发申请操作
	  ...
	  function delApplydjd(){
		...
		function djtTdiscountItem(){
			var scustsex = encodeURI(encodeURI(document.getElementsByName("sex")[0].value));
			if(Number($("#exam_id").val())>0){
	 	    	var url='djtTdiscountadd.action?exam_id='+$("#exam_id").val()+'&sex='+scustsex+'';
		...
		$.ajax({
			...
			data : {
					...
					success : function(data) {
						 ...
						 $(".loading_div").remove();
						 if(list==""||list==null){
							 $.messager.alert("操作提示", "没有卡号信息","error");
							 	$("#company_id").val("");
								$("#tjrw").val("");
								$("#batch_id").val("");
								$("#com_Name").val("");
						 }else{
							 	$("#company_id").val(list[0]["company_id"]);
								$("#tjrw").val(list[0]["batch_name"]+"("+list[0]["com_name"]+")");
								$("#batch_id").val(list[0]["id"]);
								$("#com_Name").val(list[0]["com_name"]);
						 }
					},
					error : function() {
						$(".loading_div").remove();
