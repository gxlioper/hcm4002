	...
	$('#compantmanager').tabs({
		 ...
		 onSelect : function(title) {
             if (title == bat_string1 && count != 0) {
                 var reqUrl = "companyedit.action?id="+tabselid+"";
	    ...
	    if (!$('#compantmanager').tabs('exists', bat_string1)) {
	        $('#compantmanager').tabs('add', {
	            title: bat_string1,
	            content: '<iframe scrolling="auto" width="100%" align="center" id="icompanyedit" name="icompanyedit"  onload="Javascript:SetWinHeightinn(this)"  frameborder="0"  src="companyedit.action?id='+tabselid+'"></iframe>',
	            closable: false
	        });
	    }      
	    ...
	    if (!$('#compantmanager').tabs('exists', bat_string2)) {
	        $('#compantmanager').tabs('add', {
	            title: bat_string2,
	            content: '<iframe scrolling="auto" width="100%" align="center" id="icompanyedit" name="icompanyedit"   onload="Javascript:SetWinHeightinn(this)"  frameborder="0"  src="companydwmanager.action?id='+tabselid+'"></iframe>',
	            closable: false
	        });
	    }
	    ...
	    if (!$('#compantmanager').tabs('exists', bat_string3)) {
	        $('#compantmanager').tabs('add', {
	            title: bat_string3,
	            content: '<iframe scrolling="auto" width="100%" align="center" id="icompanyedit" name="icompanyedit"  onload="Javascript:SetWinHeightinn(this)"  frameborder="0"  src="companychange.action?id='+tabselid+'"></iframe>',
	            closable: false
...
function showcomtree(data){
 ...
 function f_getMenuzbOne(patamid){
	 tabselid=patamid;
    var reqUrl = "companyedit.action?id="+patamid+"";
    refreshTab({tabTitle:bat_string1,url:reqUrl});   
    $("#compantmanager").tabs("select",bat_string1);
   }
