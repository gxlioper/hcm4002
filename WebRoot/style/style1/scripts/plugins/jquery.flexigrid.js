    ...
    $.addFlex = function(t, p) {
        ...
        var g = {
            ...
            addData: function(data) { //parse data                
                ...
                if (p.dataType == 'xml') {
                    ...
                    p.total1=+$('rows total1', data).text();
                }
                else {
                    p.total = data.total;
                    p.total1=data.total1;
                }                
                if (p.total < 0) {
                    p.total = temp;
                }
                        ...
                        $.each(data.rows, function(i, row) {
                            ...
                            $(ths).each(function(j) {
                                ...
                                //扩展列属性，class
                                	...
                                	if(this.tdClass){
                                		tbhtml.push(" class='", this.tdClass, "'");
                                	}
                               	}
            ...
            buildpager: function() { //rebuild pager based on new properties
                ...
                var stat1="";
                if(p.total1)
                	stat1+="总金额 <strong class='orange'>"+p.total1+"</strong>";
                $('.pTotal',this.cDiv).html(stat1);
            ...
            populate: function() { //get latest data 
                ...
                $.ajax({
                    ...
                    data: param,
                    dataType: p.dataType,
                    success: function(data) { if (data != null && data.error != null) { if (p.onError) { p.onError(data); g.hideLoading(); } } else { xdata=data;g.addData(data); } },
        ...
        if (p.colModel) {
            ...
            for (i = 0; i < p.colModel.length; i++) {
                ...
                //扩展列属性，class
				if(cm.thClass){
					$(th).attr('class', cm.thClass);
					th.tdClass=cm.thClass;
				}
				if(cm.tdClass)
					th.tdClass=cm.tdClass;
				if (cm.hide) {
                    th.hide = true;
                }
		...
		if(p.useTotal){
            ...
            g.cDiv.innerHTML = '<div class="cDiv2"></div>';
            $(g.bDiv).after(g.cDiv);
            var html = '<span class="pTotal"></span>';
            $('div', g.cDiv).html(html);
        ...
        //browser adjustments
        if ($.browser.msie && $.browser.version < 7.0) {
        	var ieWithd=$('.flexigrid').width()-2;
            $('.hDiv,.bDiv,.mDiv,.pDiv,.vGrip,.tDiv, .sDiv', g.gDiv)
			.css({ width: ieWithd+'px' });
            $(g.gDiv).addClass('ie6');
