...
function amountfansuan(){
	var item_amount = $('#item_amount').val();
	var amount = $('#amount').val();
	//折扣单价
	var  danjia = $('#item_amount').val()/10;
	//反算出折扣
	var fszhekou = amount/danjia;
	...
	if(fszhekou<Number($('#web_Resource').val()) ){
		$.messager.alert("提示信息","本操作人员最大权限只能打"+$('#web_Resource').val()+"折！","error");
		return;
	} else {
		$('#discount').val(decimal(fszhekou,1));
		discount(decimal(fszhekou,1));
	}
	...
	if (!rows.length == 0) {
		for (var i = rows.length - 1; i >= 0; i--) {
			if (set_numsss == rows[i].set_num) {
				...
				var row = $('#selectchangitemlist').datagrid('getRows')[index1];//获取指定索引
				if(row.ischosen!='1'){
	...
	function setfzchareitemListGrid() {
		...
		$("#selectchangitemlist").datagrid({
			...
			columns : [[ 
			   ...
			   {align : 'center',field : 'ischosen',title : '是否必选',width : 20}, 
			...
			onLoadSuccess : function(value) {
				...
				$("#selectchangitemlist").datagrid("hideColumn", "ischosen"); // 设置隐藏列  
				$("#datatotal").val(value.total);
