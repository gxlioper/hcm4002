...
function select_com_list(company_Namess){
	var url='zybcompanychangshow.action';
...
function IdCard(UUserCard,num){
	...
	function checkinput() {	
		if (document.getElementById("custname").value == '') {
			alert('姓名不能为空！');
			return false;
		}else if (($('#s_idtype').combobox('getText').trim()=="身份证")&&(document.getElementById("id_num").value.length!= '')&&(document.getElementById("id_num").value.length!= 18)) {
			alert('身份证号位数错误！');
			return false;
	...
	function findcustomer_examnum(){
		 if($("#c_exam_num").textbox('getValue')!=''){
			 var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
			 $("body").prepend(str);
			 $.ajax({
					url : 'countcustomer.action',
					data : {
						exam_num: $("#c_exam_num").textbox('getValue')
						},
						type : "post",//数据发送方式   
						success : function(text) {
							    $(".loading_div").remove();
	...
	function f_custdingweishow(){ 
		 ...
		 var strurl='findcustOneDingwei.action?dingweitype='+contralcustflag+'&exam_num='+$("#c_exam_num").textbox('getValue')+'&custname='+scustname+'&id_num='+$("#id_num").val();
		 $("#dlg-show").dialog({
		 		title:'条件定位',
		 		href:strurl
		 	});
	...
	//新版本打印导检单
	 ...
	 function djtTdiscountItem(){
			var scustsex = encodeURI(encodeURI(document.getElementsByName("sex")[0].value));
			if($("#exam_num").val()){
	 	    	var url='zybdjtTdiscountadd.action?exam_num='+$('#exam_num').val()+'&sex='+scustsex+'';
...
//			 	$.messager.alert('提示信息',"无效付费状态！","error");
				...
				var url='zybdjtcustomerGitemaddshow.action?exam_num='+$("#exam_num").val()+'&sex='+scustsex+'';
	 	    	newWindow = window.open(url, "团体职业病加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
	 	    	newWindow.focus();
			}
		}
	 function zybaddcusItemG(){
			var scustsex = encodeURI(encodeURI(document.getElementsByName("sex")[0].value));
			if($('#exam_num').val()==''){
			 	$.messager.alert('提示信息',"请先确定体检人员！","error");
			}else if($("#examstatus").val() == 'Z'){
...
//			 	$.messager.alert('提示信息',"无效付费状态！","error");
				...
				var url='zybdjtcustomeritemaddshow.action?exam_num='+$("#exam_num").val()+'&sex='+scustsex+'';
	 	    	newWindow = window.open(url, "个人职业病加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
	 	    	newWindow.focus();
			}
		}
...
//		 	$.messager.alert('提示信息',"无效付费状态！","error");
			...
			var url='zybdjtTTcustomeritemaddshow.action?exam_num='+$("#exam_num").val()+'&sex='+scustsex+'&exam_indicator=T';
	    	newWindow = window.open(url, "团体普通加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
	    	newWindow.focus();
	    	//timer = setInterval("djtupdateAfterClose()", 1000);
 	   	}
	}
...
//		 	$.messager.alert('提示信息',"无效付费状态！","error");
			...
			var url='zybdjtTGcustomeritemaddshow.action?exam_num='+$("#exam_num").val()+'&sex='+scustsex+'&exam_indicator=G';
 	    	newWindow = window.open(url, "个人普通加项", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
 	    	newWindow.focus();
 	    	//timer = setInterval("djtupdateAfterClose()", 1000);
 	   	}
	}
						 ...
						 success : function(text) {
							...
							if(text.indexOf("error")==0){
								$.messager.alert("操作提示", text.split("-")[1], "error");
		  						...
		  						$("#c_exam_num").textbox('setValue',text.split("-")[1]);
		  						findcustomer_examnum();
		  					}else{
...
var occtoolbar = [{
    ...
    handler: function () {
        if ((document.getElementById("id_num").value == '') && (document.getElementById("exam_num").value == '') && (document.getElementById("arch_num").value == '')) {
            ...
            var url='zybCustomerHazard.action?exam_num='+$("#c_exam_num").val()+'&arch_num='+document.getElementById("arch_num").value
