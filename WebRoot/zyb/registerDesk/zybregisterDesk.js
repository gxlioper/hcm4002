...
//    	    });
	    ...
	    handler:function(){
	 	...
	 	iconCls:'icon-save',
	 	handler:function(){
			 window.parent.addPanel_other('职业病个人登记','getZybDjtRegisterGList.action?exam_num=','"+wx.getIcon_url()+"','1');
	 	}
 	},{
		text:'预约改期',
		width:90,
		...
		handler:function(){
    	    ...
    	    if((exam_num=="")||(exam_num.split(',').length!=1)){
	    		...
	    		if(examstatus=='Y'){
	    		    ...
	    		    if(examtype=='G'){
	    			   window.parent.addPanel_other("复查个人登记","getZybDjtRegisterGEdit.action?exam_num="+exam_num+"&ren_type=1","","1");
	    		    }if(examtype=='T'){
	    			   window.parent.addPanel_other("复查团体登记","getZybDjtRegisterTListRecheck.action?exam_num="+exam_num+"&ren_type=1","","1");
	    		    }
	    		}
	    	}
	    }
