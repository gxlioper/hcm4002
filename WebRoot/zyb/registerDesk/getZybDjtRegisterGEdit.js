...
$(document).ready(function () {
    ...
    f_findcustomerone($("#c_exam_num").val(), 0);
    $("#birthday").datebox({
...
function fnewbutton(vcontralcustflag) {
    ...
    if ((contralcustflag == 0) && ($("#c_exam_num").val().length > 0)) {
        $.messager.alert("操作提示", "记录存在，不能新增", "error");
    } else if ((contralcustflag == 1) && ($("#c_exam_num").val().length) > 0) {
        $.messager.alert("操作提示", "记录存在，不能预约", "error");
    } else if ((contralcustflag == 2) && ($("#c_exam_num").val().length) <= 0) {
...
function addcustomerdo() {
    ...
    $("body").prepend(str);
    $.ajax({
        url: 'djtzybeditcustomerdo.action',
        data: {
            customer_id: $("#customer_id").val(),
            custname: document.getElementById("custname").value,
            id_num: document.getElementById("id_num").value,
            exam_id: $("#exam_id").val(),
            exam_num: document.getElementById("c_exam_num").value,
...
function f_findcustomerone(c_exam_num, brushtatle) {
...
function djtdeletetc(userid) {
    if (($("#c_exam_num").val() == '') || (Number($("#c_exam_num").val()) <= 0)) {
...
function djtcustChangItemListGrid() {
    var model = {"exam_num": $("#c_exam_num").val()};
...
var toolbar = [{
    ...
    iconCls: 'icon-check',
    handler: function () {
        if ($("#c_exam_num").val() == '' || $("#c_exam_num").val() == '') {
...
function zybaddcusItem() {
    ...
    if (Number($("#exam_id").val()) > 0) {
        var url = 'zybdjtcustomeritemaddshow.action?exam_num=' + $("#c_exam_num").val() + '&sex=' + scustsex + '';
...
function djtaddcusItem() {
    var scustsex = encodeURI(encodeURI(document.getElementsByName("sex")[0].value));
    if (Number($("#exam_id").val()) > 0) {
        var url = 'djtcustomeritemaddshow.action?exam_num=' + $("#c_exam_num").val() + '&sex=' + scustsex + '';
...
{
    if (($("#c_exam_num").val() != '') && ($("#c_exam_num").val().length > 0)) {
        var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
        $("body").prepend(str);
        $.ajax({
                ...
                if (r) {
                    ...
                    $
                        .ajax({
                            ...
                            data: {
                                exam_num: document
                                    .getElementById("exam_num").value,
                                id_num: document
                                    .getElementById("id_num").value,
                                arch_num: document
                                    .getElementById("arch_num").value,
                                ids: delzywhlbid,
                                exam_id: $('#exam_id').val()
                            },
                            type: "post",// 数据发送方式
