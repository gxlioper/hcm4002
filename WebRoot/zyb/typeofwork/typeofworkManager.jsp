...
<script type="text/javascript">
	...
	/**
	  ...
	  */
	  var toolbar = [{
		    ...
		    handler:function(){
		    	var ids = new Array();
		    	var checkedItems = $('#tyofwork').datagrid('getChecked');
			    $.each(checkedItems, function(index, item){
			       ids.push("'"+item.typeofworkID+"'");
			    }); 
	...
	//批量删除
	    function f_deletes(id) {
	    		...
	    		$.messager.confirm('确认','您确认要删除记录吗？',function(r){    
	    		    if (r){ 
	    		    	$.ajax({
	    		    		url : 'deletetypeofworks.action?ids='+id,
	    		    		type : 'post',
	    		    		success : function(data) {
	    		    			$.messager.alert('提示信息',data,'info');
	    		    			$('#tyofwork').datagrid('reload');
	    		    		},
	    		    		error : function() {
	...
	function updateTofW(id){
	    $("#edit").dialog({
			title:'修改',
			href:'updatetypeOfWork.action?typeofworkID='+id
		});
		$("#edit").dialog('open'); 
	}
	//删除
	function deleteTofW(id){
		$.messager.confirm('提示信息','是否确定删除？',function(r){
			if(r){
				$.ajax({
		   		url:'deletetypeOfWork.action?typeofworkID='+id,
		   		type:'post',
		   		success:function(data){
...
<body class="easyui-layout"> 
    ...
    <div id="edit" class="easyui-dialog"  data-options="width: 500,height: 370,closed: true,cache: false,modal: true,top:50"></div>  
</body> 
</html>