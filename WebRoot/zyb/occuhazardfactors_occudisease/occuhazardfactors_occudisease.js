 ...
 */
 function getOccuhazardfactorsList(){
   ...
   /*  var str = '<div class="loading_div" style=" position:fixed; background-image:url(../../images/bg_tm.png); left:0;top:0; overflow:hidden; width:100%; height:100%; display: block; z-index: 89999; ">' + '<div style="width:70px;  height:75px; margin-top:-38px; position:absolute; top:50%; left:50%; margin-left:-35px;"><img src="../../images/loading.gif" width="70" height="75" /></div></div>';
		 ...
		 url:'getZyboccudiseaseYinsuTable.action',
		 queryParams:{
			 occudisease_name:$('#contraindication_name').val(),
			 tjlb:z,
			 hazardfactorsID:f
		 },
		 toolbar:toolbar,
		...
		// height:'',
		 ...
		 columns:[[
	            ...
	            {align:'left',field:'occudisease_name',title:'疾病名称',width:30},
	             {align:'left',field:'hazard_name',title:'危害因素',width:40},
			 	{align:'center',field:'occuphyexaclass_name',title:'体检类别',width:40},
 ...
 */
 function updateZYB_OccuphyexaPage(id){
			...
			height:385,
			center:'center',
			href:'getupdateZyboccudiseaseYinsuPage.action?RID='+id
		});
...
function f_deluser(id) {
	...
	$.messager.confirm('确认','您确认要删除记录吗？',function(r){    
	    if (r){ 
	    	$.ajax({
	    		url : 'getDeleteZyboccudiseaseYinsu.action?ids='+id,
	    		type : 'post',
...
var toolbar = [ {
	...
	handler : function() {
		...
		$('#lbname').val(node.text);
		$('#ysname').val(f_lx.text);
		//alert(f_lx.id);
		if(lx=="lx"){
			$("#dlg-custedit").dialog({
				title : '新增',
				center:'center',
				href :'getAddZyboccudiseaseYinsuPage.action?tjlb='+node.id+'&hazardfactorsID='+f_lx.id
			});
			$("#dlg-custedit").dialog('open');
		} else {
