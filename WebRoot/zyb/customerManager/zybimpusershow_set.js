	 ...
	 $("body").prepend(str);
		 ...
		 columns:[[
		    ...
		    {align:'center',field:'visit_no',title:'就诊卡',width:15,editor:{type:'text'}},
		 	...
		 	{align:'center',field:'joinDatetime',title:'进厂时间',width:15,editor:{type:'text'}},	
		 	...
		 	{align:'center',field:'employeeage',title:'总工龄',width:15,editor:{type:'text'}},
		 	{align:'center',field:'damage',title:'接害工龄',width:15,editor:{type:'text'}},
		 	{align:'center',field:'exam_type',title:'职业体检类型',width:15,editor:{type:'text'}},
		 	{align:'center',field:'zywhys',title:'危害因素类别',width:15,editor:{type:'text'}},
		 	{align:'center',field:'sets',title:'套餐',width:15,editor:{type:'text'}},
		 	{align:'center',field:'idtypename',title:'证件类型',width:15,editor:{type:'text'},sortable:true},
		 	{align:'center',field:'exam_indicator',title:'付费类别',width:15,editor:{type:'text'},sortable:true},
		 	{align:'center',field:'exam_class',title:'体检类型',width:15,editor:{type:'text'},sortable:true},
		 	{align:'center',field:'remark',title:'备注',width:80,editor:{type:'text'},sortable:true},
		 	{align:'center',field:'others',title:'其他',width:80,editor:{type:'text'},sortable:true},
		 	]],	    	
 ...
 var toolbar=[{
	    ...
	    handler:function(){
	    	if(($("#company_id").val()=='')||(Number($("#company_id").val())<=0)||($("#batch_id").val()=='')||(Number($("#batch_id").val())<=0)){
	    		...
	    		$("#dlg-show").dialog({
		 		title:'上传文件',
		 		href:'zybimpuserfile.action?batch_id='+$("#batch_id").val()+'&company_id='+$("#company_id").val()
		 	});
		...
		text:'<a href=\"../../zyb/customerManager/zybtemplate_set.xls\">下载模板(关联自选套餐)</a>',
		iconCls:'icon-check',
		handler:function(){
	    }
	}];
...
function getBatchById(callback){
 ...
 function impuser(ids){
	...
	$("body").prepend(str);
	 $.ajax({
			...
			url : 'zybimpuserToExaminfodo.action',
			data : {
 ...
 function savrow(row){
	 ...
	 $("body").prepend(str);
	 $.ajax({
			url : 'zybsaveCustomerTmplist.action',
			data : {
						id : row.id,
