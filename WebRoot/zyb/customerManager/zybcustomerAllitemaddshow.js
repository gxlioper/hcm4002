...
function countamt3() {
	...
	if (checkinput()) {
		...
		$.ajax({
			url : 'zybcustomeritemadddo.action',
			data : {
				ids : $("#ids").val(),
	...
	function setfzchareitemListGrid() {
		...
		$("#selectchangitemlist").datagrid({
			...
			columns : [[ 
			   ...
			   {align : 'center',field : 'discount',title : '折扣率',hidden : true,width : 20,editor : {type : 'text'}}, 
			   {align : 'center',field : 'amount',title : '金额',hidden : true,	width : 20,	editor : {type : 'text'}} 
			   ]],
			onLoadSuccess : function(value) {
				$("#datatotal").val(value.total);
			},
