...
$(document).ready(
		function() {
			...
			if(is_show_discount==0){
				$('#ttpt_zkl').hide();// 设置隐藏列
				$('#ttpt_zkhze').hide();// 设置隐藏列
				$('#discount').hide(); // 设置隐藏列
				$('#amount').hide();// 设置隐藏列
				$('#ttpt_yuan').hide();// 设置隐藏列
			}
			setselectGrid();
...
function countamt() {
	...
	for(var i=0;i<personal_pay_inputss.length;i++)
		...
		tamt = tamt + Number(team_pay_inputss[i].value);
		}
	}
	...
	$.post("setzybforchangitemlist.action", {
		...
		for (var i = 0; i < userid.length; i++) {
			if (!isFloat($("#discount").val())) {
				...
				userid[i].amount = decimal(userid[i].item_amount
						* $("#discount").val() / 10, 2);
				userid[i].amount = decimal(userid[i].item_amount*userid[i].itemnum
						* userid[i].discount / 10, 2);
				deltiemflags = 1;
...
function djtcustadd() {
	...
	if (itemrows.length > 0) {
		...
		$.ajax({
			url : 'zybTTcustSaveItemSet.action',
			data : {
				exam_id : $("#exam_id").val(),
				exam_num:$('#exam_num').val(),
