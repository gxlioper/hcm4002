...
$(document).ready(function(){
	...
	if($("#status").val() == '3'){
		...
		for(i=0;i<obj.length;i++){
			...
			}else if(obj.eq(i).children().children().html() == '历史结果对比'){
				obj.eq(i).children().css('margin-top','-2px');
...
//判断是否打开问诊页面
function isOpenWenZhen(pra){
	var dep_id = $("#dep_id").val();
	var url="";
	var code = "";
	var mess = "";
	if(pra == 'ZYJKJC'){
		url='brainShowPage.action?quest_sub_code=QS06&exam_num='+$("#tjh").html()+"&dep_id="+dep_id;
		var code = "QS06";
		mess = "非放射性问诊";
	}else if(pra == 'FSJKJC'){
		url='brainShowPage.action?quest_sub_code=QS07&exam_num='+$("#tjh").html()+"&dep_id="+dep_id;
		var code = "QS07";
		mess = "放射性问诊";
	}
	$.ajax({
...
function getexamIntion(){
	$.ajax({
		url:'getDepInspectExamIntion.action?exam_num='+$('#exam_num').val()+'&sex='+$('#sex').text()+'&app_type='+$("#app_type").val(),
		success:function(data){
			samp_item = eval('('+data+')');
...
function zhi(va,id,brief){
	...
	for(i=0;i<item_list.length;i++){
		if(item_list[i].id == id){
			if(item_list[i].default_value == $("#item_"+id).val() || item_list[i].default_value == arr || $("#item_"+id).val() == '' || $("#item_"+id).val() == '婉拒检查'){
				$("#item_"+id).val(arr);
...
function addexamDepResult(){
	...
	$("#dt li").each(function(){
		...
		ms.push({
			...
			"exam_result":$(this).find(".result").val().trim(),
			"health_level":zt,
