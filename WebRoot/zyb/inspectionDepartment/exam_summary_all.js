$(document).ready(function () {
	showtab();	
	var height = window.screen.height-112;  
	...
	$('#getDjtRegisterGList').tabs({
	 ...
	 if (!$('#getDjtRegisterGList').tabs('exists', bat_string1)) {
		 $('#getDjtRegisterGList').tabs('add', {
			 title: bat_string1,
			 content: '<iframe scrolling="auto" width="100%" height="100%" align="center" id="igetDjtRegisterGedit" name="igetDjtRegisterGedit"  onload="Javascript:SetWinHeightinn(this)" frameborder="0"  src="getExamSummaryPage.action?exam_num='+$("#exam_num").val()+'&status=3"></iframe>',
			 closable: false
		 });
	 }     
	 if (!$('#getDjtRegisterGList').tabs('exists', bat_string2)) {
		 $('#getDjtRegisterGList').tabs('add', {
			 title: bat_string2,
			 content: '<iframe scrolling="auto" width="100%" height="100%" align="center" id="igetDjtRegisterGedit" name="igetDjtRegisterGedit"   onload="Javascript:SetWinHeightinn(this)" frameborder="0"  src="getZybExamSummaryPage.action?exam_num='+$("#exam_num").val()+'&status=3"></iframe>',
			 closable: false
		 });
	 }
	 $("#getDjtRegisterGList").tabs("select",bat_string1);
...
function showOnetab(tab_index,tab_url){
 ...
 function SetWinHeightinn(obj) {
	 var height = window.screen.height-122;  
	 obj.height = height;
	} 
