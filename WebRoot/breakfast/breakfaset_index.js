...
function getExamInfoByNum(){
	if($(".pagination-info").html() != undefined){
		...
		$.ajax({
			url:'getBreakFastIndexList.action?exam_num='+exam_num,
			success:function(data){
				...
				for(i=0;i<exam.rows.length;i++){
					...
					if(flag){
						...
						f_examoneshow(exam.rows[i].exam_num);
					}
				}
			}
		});
...
function getroupGridByNum(){
	var exam_num = undefined;
	 if($("#ck_exam_num")[0].checked){
		 ...
		 url:'getBreakFastIndexList.action',
		 dataType: 'json',
		 queryParams:model,
		 columns:[[
           {field:'ck',checkbox:true },
		    ...
		    rowStyler:function(index,row){   
		        if (row.exam_status=='N' || row.exam_status=='D'){   
		            return 'color:red;';   
		        }   
		    }   
	});
...
/**
 ...
 function getgroupuserGrid(){
   ...
   function f_examoneshow(id){
	   var url='breakfast.action?exam_num='+id;
	   newwin = window.open("", "早餐科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
	   newwin.moveTo(0,0);
