...
function getteamExamInfoShowGridzyb() {
	...
	var chk_value ="";    
	 ...
	 if(chk_value.indexOf("company_id") < 0 && chk_value.indexOf("batchid") < 0 ){
		 chk_value = chk_value + ",exam_num";
	 }
	...
	$("#teamExamInfoShow").datagrid({
		url:'queryCommonReportTypeList.action',
		dataType : 'json',
		queryParams : model,
		remoteSort:false,
		toolbar:'#toolbar',
		...
		pageList : [ 200,300,500 ],// 可以设置每页记录条数的列表
		columns : [[  {field:'ck',checkbox:true },
					  {align : 'center',field : 'id',title : 'ID',width : 15,sortable:true,hidden:true},
		              {align:'center',field:'examreport',title:'报告',"formatter":f_reportshowzyb},
		              {align : 'center',field : 'exam_num',title : '体检编号',width : 18,"formatter":f_showexam,sortable:true}, 
		              {align : 'center',field : 'arch_num',title : '档案号',width : 25,sortable:true}, 
		              {align : 'center',field : 'id_num',title : '身份证号',width : 25,sortable:true}, 
		              {align : 'center',field : 'user_name',title : '姓名',width : 15,sortable:true}, 
		              {align : 'center',field : 'sex',title : '性别',width : 10,sortable:true}, 
		              {align : 'center',field : 'age',title : '年龄',width : 10,sortable:true}, 
		              {align : 'center',field : 'phone',title : '电话',width : 15,sortable:true},
		              {align : 'center',field : 'exam_types',title : '类型',width :15,sortable:true},
		              {align : 'center',field : 'tjlx',title : '体检类型',width :15,sortable:true},	
		              {align : 'center',field : 'comonReportType',title : '报告类型',width :15,sortable:true},
		              {align : 'center',field : 'statussss',title : '体检状态',width :15,"formatter":f_showstatus,sortable:true},
...
//		              {align : 'center',field : 'zywhysname',title : '职业危害',width : 47,sortable:true}
	...
	var toolbazybr=[{
			...
			iconCls:'icon-edit',
			handler:function(){
		    	var rowInfo = $("#teamExamInfoShow").datagrid('getSelected'); //获得选中行
		    	var rowIndex=$('#teamExamInfoShow').datagrid('getRowIndex',rowInfo);//行号
		    	if(rowInfo){
...
function updateReportEdit(){
	...
	$.ajax({
		url:'updateReportTypeEdit.action',
		data:{
			ids:item_ids.toString(),
			Comon_report_type:$("#commonReportType").combobox('getValue')
		},
		type:'post',
		success:function(data){
			var obj=eval("("+data+")");
