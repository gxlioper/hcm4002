...
function countamt3() {
	...
	if($("#isDefaultTen").val()=="Y"){
		...
		$("#discount").val(decimal(Number(tamt+gamt) / Number(itemamout) * 10, 2));
	}
}
...
function setfzchareitemListGrid() {
	...
	$("#selectchangitemlist").datagrid({
		...
		columns : [ [ 
		    ...
		    {align : 'left',field : 'item_code',title : '项目编码',width : 26}, 
...
//		    {align : 'left',field : 'dep_name',title : '科室',width : 25}, 
		    ...
		    {align : 'center',field : 'discount',title : '折扣率',width : 20,"formatter" : f_discount}, //刘翔
		    {align : 'center',field : 'exam_indicators',title : '付费方式',width : 30,	"formatter" : f_exam_indicators},
		    {align : 'center',field : 'team_pay',title : '单位金额',width : 25,"formatter" : f_team_pay}, 
		    {align : 'center',field : 'personal_pay',title : '个人金额',width : 26,"formatter" : f_personal_pay}
		    ] ],
		onLoadSuccess : function(value) {
...
function f_discount(val, row) {
	if (deltiemflags == 0) {
		...
		return '<input type=\"text\" name=\"discount_input\" id=\"discount_input'
				+ row.item_code
				+ '\" maxlength=\"3\" size=\"3\"  onclick=\"clikezhekou(this.value);\" onblur=\"f_discount_input(\''
				+ row.item_code
				+ '\','
				+ row.item_amount
...
function f_team_pay(val, row) {
	if (deltiemflags == 0) {
		...
		return '<input type=\"text\" name=\"team_pay_input\" id=\"team_pay'
				+ row.item_code
				+ '\"  maxlength=\"10\" size=\"5\" onchange=\"f_team_pay_input(\''
				+ row.item_code + '\',' + row.item_amount
...
function f_team_pay_input(row_item_code, row_item_amt, team_pay_input) {
	...
	if (isNaN(Number(team_pay_input))) {
		...
		if( $('#web_Resource').val()!=undefined && $('#web_Resource').val()!=""){
			zhekoulvqx=decimal(Number($('#web_Resource').val()),1);
		}
		...
		if (Number(team_pay_input) > depositValue) {
			if(exam_indicators_input=='T'){		
				var dzsize = decimal(Number(team_pay_input)/Number(invMoneyValue)*Number(itemnumValue) * 10, 1);
				...
				var dzsize = decimal((Number(team_pay_input)+Number(personal_pay_input))/Number(invMoneyValue)*Number(itemnumValue) * 10, 1);
				if((zhekoulvqx>dzsize)||(dzsize>10)){
					$('#team_pay' + row_item_code).val(decimal(Number(invMoneyValue)*Number(itemnumValue), 2));
					$('#personal_pay' + row_item_code).val(0);
					$('#discount_input' + row_item_code).val(10);
...
function f_personal_pay(val, row) {
	if (deltiemflags == 0) {
		...
		return '<input type=\"text\" name=\"personal_pay_input\" id=\"personal_pay'
				+ row.item_code
				+ '\"  maxlength=\"10\" size=\"5\" onchange=\"f_personal_pay_input(\''
				+ row.item_code
				+ '\','
				+ row.item_amount
...
function f_personal_pay_input(row_item_code, row_item_amt, personal_pay_input) {
	...
	if (isNaN(Number(personal_pay_input))) {
		...
		if (Number(personal_pay_input) > depositValue) {
			if(exam_indicators_input=='T'){
				var dzsize = decimal(Number(personal_pay_input)/Number(invMoneyValue)*Number(itemnumValue) * 10, 1);
				if((zhekoulvqx>dzsize)||(dzsize>10)){
					$('#team_pay' + row_item_code).val(decimal(Number(invMoneyValue)*Number(itemnumValue), 2));
					$('#personal_pay' + row_item_code).val(0);
					$('#discount_input' + row_item_code).val(10);
