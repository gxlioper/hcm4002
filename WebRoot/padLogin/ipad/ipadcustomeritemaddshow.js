		...
		$("#changitemlist").datagrid(
				{
					...
					columns : [[ {align : 'left',	field : 'item_code',title : '项目编码',width : 16},
					...
					striped : true,
					onLoadSuccess: function (data) {
			            var s = $("#changitemlist").datagrid('getPanel');
			          var rows = s.find('tr.datagrid-row');                       //注意这一行
	...
	function setfzchareitemListGrid() {
		...
		$("#selectchangitemlist").datagrid({
			...
			columns : [[ 
			   ...
			   {align : 'left',field : 'item_code',title : '项目编码',width : 20}, 
			   ...
			   {align : 'left',field : 'dep_name',title : '科室',width : 23}, 
			   {align : 'left',field : 'id',title : 'ID',width : 15,hidden:true},
			   {align : 'left',field : 'sex',title : '性别',width : 15,hidden:true},
			   {align : 'center',field : 'item_amount',title : '原金额',width : 17},
			   {align : 'center',field : 'itemnum',title : '数量',width : 12,"formatter" : f_itemnum}, 
	...
	function f_personal_pay(val, row) {
		if (deltiemflags == 0) {
			...
			return '<input type=\"text\" name=\"personal_pay_input\" id=\"personal_pay'
					+ row.item_code
					+ '\"  maxlength=\"10\" size=\"5\" onclick=\"x_jineFinal(this)\" onchange=\"f_personal_pay_input(\''
					+ row.item_code
					+ '\','
					+ row.item_amount
