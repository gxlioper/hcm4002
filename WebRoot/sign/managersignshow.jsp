...
<html>
			...
			function get_info()
			{
				...
				if("" == scriptImage){
					...
					if("" == ResizeImage){
						...
						$.ajax({
							url : 'managersignupdate.action',
							data : {
								id : $("#userid").val(),
								user_signature : sign_value,
								remark1 : ResizeImage
							},
							type : "post",// 数据发送方式
							success : function(text) {
								if (text.split("-")[0] == 'ok') {
									...
									document.getElementById("img_script").src="getuserPhoto.action?id="+text.split("-")[1]+"&"+new Date().getTime();
								} else {
									ShowMsg(text.split("-")[2]);
								}
							},
	...
	<body onLoad="LoadOcx()">	
	<input id="userid" type="hidden" value="<s:property value="id"/>"/>
	<input id="txt_signer" type="hidden" value="<s:property value="chi_Name"/>"  />
	...
	<input id="txt_plain" type="hidden" value="<s:property value="chi_Name"/>"  /> 		
	<input id="txt_sign_value" type="hidden" value="<s:property value="user_signature"/>" />
				...
				<tr>
					<td>
						<div id="script_image"  title="此处显示最后获取到的签名图片"
							...
							<img type="text" style="width:188px;height:188px;" id="img_script" src='getuserPhoto.action?id=<s:property value="id"/>'></img>
						</div>
					</td>
					<td>
