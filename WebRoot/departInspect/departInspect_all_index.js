...
$(document).ready(function(){
    ...
    $("#hydiv").hide();
    $("#yxdiv").hide();
    $('#t_exam_num').textbox('textbox').focus();

    $('#t_exam_num').textbox('textbox').keydown(function (e) {
        if (e.keyCode == 13) {
            getcustomerInfo();
        }
    });
...
function getcustomerInfo(){
    $.ajax({
        url:'getCustomerInfo.action?exam_num='+$("#t_exam_num").val(),
        type:'post',
        async:false,
        success:function(data){
            ...
            if(data!="null"){
                var obj=eval("("+data+")");
                $("#id").val(obj.id);
                $("#tjh").html(obj.exam_num);
                $("#arch_num").html(obj.arch_num);
                $("#u_name").html(obj.user_name);
                $("#sex").html(obj.sex);
                $("#ages").html(obj.age);
...
function create_item(obj){
    ...
    $("#dt").html(row);
    var inputter = getCookie($("#dep_id").val()+"-inputter");
    if(obj[0].inputter > 0){
...
//		dataType: "json",
        ...
        success:function(data){
            ...
            if(jcxm.length>0){
                ...
                var top = $(ths).offset().top+height-65;
...
function savedepresult(charing_ids,ms){
    $.ajax({
        ...
        success:function(data){
            $(".loading_div").remove();
            setCookie($("#dep_id").val()+'-inputter',$('#inputter').combobox('getValue'));
                ...
                for(i=0;i<obj.length;i++){
                    if(sam_demo_id > 0 && obj[i].sample_id == sam_demo_id){
                        ...
                        str += '<div class="in_conclusion">'+
                            ...
                            // '<span class="l-btn-left" style="margin-top: 0px;"><span class="l-btn-text"></span></span></a>'+
                            '<a href="javascript:void(0)" class="easyui-linkbutton c6 l-btn l-btn-small" style="width:60px;float: right;margin-right: 6%;" onclick="javascript:saveViewExamDetail('+i+');">'+
    ...
    if(obj == undefined || obj == null){
        ...
        $.ajax({
            ...
            success:function(data){
                $(".loading_div").remove();
                var objq = data.split("-");
                setCookie($("#dep_id").val()+'-inputter',$('#yx_inputter').combobox('getValue'));
                if(objq[0] == 'ok'){
                    dep_shu();
                    if(obj.length == 1){
...
function getJYitem(item_id){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $("#hy").html(ro);
            $("#hy_item_0").focus();
            var tt = $("#dep_id").val()+"-inputter";
            var inputter = getCookie(tt);
...
function saveExam_result(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            var tter = $("#dep_id").val()+'-inputter';
            setCookie(tter,$('#hy_inputter').combobox('getValue'));
            var obj = data.split("-");
            if(obj[0] == 'ok'){
