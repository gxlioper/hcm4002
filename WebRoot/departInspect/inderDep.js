	   ...
	   $.ajax({
			...
			success : function(text) {
					if (text == '1') {//健康体检
						...
						var url='inspectshow.action?exam_num='+exam_num;
						newWindow = window.open(url, "健康体检科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
						newWindow.focus();
					}else if(text == '2'){//职业病
						$("#exam_num").textbox("setValue", "");
						var url='zybInspectshow.action?exam_num='+exam_num;
						newWindow = window.open(url, "职业病体检科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
						newWindow.focus();
					}else if(text == '3'){//健康体检和职业病
						$("#exam_num").textbox("setValue", "");
						var url='inspectshowAll.action?exam_num='+exam_num;
						newWindow = window.open(url, "科室检查", "toolbar=no,location=no,fullscreen=yes,scrollbars=yes");
						newWindow.focus();
					}else {
