	...
	//回车查询事件
	 ...
	 $('#screater').combobox({
			...
			onLoadSuccess : function(data) {
				$('#screater').combobox('setValue', $("#userid").val());
			}
		});
...
function gettjbg(){
	var model = {exam_num:$("#exam_num_s").val(),app_type:""};
  ...
  function quxiaoshanngchuan(){
		...
		if ($('#exam_num').val() > 0) {
			$.ajax({
				...
				success : function(text) {
					...
					} else if(text.split("-")[0] == 'tishi'){
						$.messager.alert("错误信息", text.split("-")[1], "error");
					}else{
	...
	function getlispacs() {
		if ($("#exam_num_s").val() != '') {
			...
			$.messager.alert("操作提示", "无效体检编号", "error");
		}
	}
...
<input type="hidden" id="upload_flow1" value="<s:property value="model.upload_flow"/>">
<input type="hidden" id="wjItem_dep" value="<s:property value="model.wjItem_dep"/>">
 			...
 			<fieldset style="margin:5px;padding-right:0;">
    			<legend><strong>基本信息</strong></legend>
					...
					<input type="hidden" name="exam_num" id="exam_num"/>
						...
						<%--<dl><dt style="width:80px;">危机提示：</dt><dt id="weiji" style="color: red" ></dt></dl>--%>
   ...
   <!--右边面版  -->
		 <div id="right">
    			...
    			<legend><strong>信息检索</strong></legend>
					<div class="user-query">
						<dl>
							...
							<dd><a href="javascript:chaxun();gettjbg();" class="easyui-linkbutton c6" id="qz"   style="width: 90px">接收导引单</a></dd>
							...
							<!--<dd id = "upload"><a href="javascript:jieshoukeshangchuan();" class="easyui-linkbutton c6" id="ksc" style="width: 90px">可上传</a></dd>
        					<dd id = "removeUpload"><a href="javascript:quxiaoshanngchuan();" class="easyui-linkbutton"   style="width: 90px">取消上传</a></dd>-->
        					<dd id = "upload_flow"><a href="javascript:uploadFlow1();" class="easyui-linkbutton c6"  style="width: 90px">上传总检</a></dd>
							<!-- 
 			...
 			<fieldset style="margin:5px;padding-right:0; padding-top: 5px; padding-bottom: 20px; height: 520px; margin-right: 10px" >
    			<legend><strong>信息显示</strong>&nbsp;&nbsp;
					<%--<span style="background-color:#f43ba1;">&nbsp;&nbsp;&nbsp;</span>
                    <span style="font-size:14px;">--危机值提示 &nbsp;&nbsp;</span>--%>
    			</legend>							
	...
	<fieldset style="margin:5px;padding-right:0; padding-top: 5px; padding-bottom: 20px; height: 110px; margin-right: 10px" >
    			<legend><strong>信息检索</strong></legend>
					...
					<dd><input type="text" style="width:215px" id="username" class="textinput"/></dd>
		    	  	<dt style="height:26px;width:80px;"><input id="ck_screater" type="checkbox" checked="checked"/>查询人员</dt>	
...
<script type="text/javascript" src="<%=request.getContextPath()%>/examflow/flow_s_show1.js?randomId=<%=Math.random()%>"></script>