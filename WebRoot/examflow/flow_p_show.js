...
5         */
 ...
 ///////////////////////////////////////上传////////////////////////////////////////////////////////////////////
   ...
   function shangchuanchaxun(){
		 ...
		 if($("#printer")[0].checked){
		 	printer = $("#printers").combobox('getValue');
		 }
		 
		 var printTime1 = undefined;
		 var printTime2 = undefined;
		 if($("#printTime")[0].checked){
		 	printTime1 = $("#printTime1").datebox('getValue');
			printTime2 = $("#printTime2").datebox('getValue');
		 }
		 
		 var printFlag = undefined;
		 if($("#printStatus")[0].checked){
		 	printFlag = $("#printFlag").combobox('getValue');
		 }
		/* var is_vip = undefined;
		 if($("#ck_is_vip")[0].checked){
			 is_vip = $('#is_vip').combobox('getValue');
		 }
		 var health_report = undefined;
		 if($("#ck_health_report")[0].checked){
			 health_report = $('#is_health_report').combobox('getValue');
		 }*/
	   var model={
		 "exam_num":exam_num,
		 "time1":time1,
		 ...
		 "set_id":set_id,
		 "printer":printer,
		 "printTime1":printTime1,
		 "printTime2":printTime2,
		 "printFlag":printFlag
//		 "isVip":is_vip,
//		 "health_report":health_report
	   };
	     $("#flowexamlist").datagrid({
		 url:'flowexampListp.action',
		 dataType: 'json',
		 queryParams:model,
		 toolbar:'#toolbar',
		 pageNumber:1,
		 ...
		 columns:[[
		    ...
		    {align:'center',field:'exam_types',title:'体检类型'},	
		 	...
		 	{align:'center',field:'join_date',title:'体检日期',sortable:true},	 	
		  	...
		  	{align:'center',field:'printerName',title:'打印人',sortable:true},
		  	{align:'center',field:'printdate',title:'打印时间',sortable:true},
		 	{align:'center',field:'strp0',title:'整单接收状态'},
		 	{align:'center',field:'p0date',title:'整单接收时间',sortable:true},		 	
		 	{align:'center',field:'strp1',title:'整单上传状态'},
		 	{align:'center',field:'p1date',title:'整单上传时间',sortable:true}
		 	]],	    	
	    	onLoadSuccess:function(value){ 
	    		$("#datatotal").val(value.total);
	    		$(".loading_div").remove();
 ...
 //批量转移
   ...
   function gethealthreport(){
	   ...
	   var checkedItems = $('#flowexamlist').datagrid('getChecked');
	    ...
	    if(listh1.length>0){
	    	...
	    	$("body").prepend(str);
	    	$.ajax({
				url : 'getHealthReportPath.action',
				data : {'exam_num' : listh1.toString(),'ptype':1},
				type : "post",//数据发送方式   
				success : function(text) {
					$(".loading_div").remove();
					shangchuanchaxun();
   ...
   function printhealthreport(){
	   ...
	   var checkedItems = $('#flowexamlist').datagrid('getChecked');
	    $.each(checkedItems, function(index, item){
	    	if(item.marker == '无'){
	    		$.messager.alert("操作提示", "体检人("+item.user_name+")没有健管报告,不能打印!", "error");
	    		return;
	    	}
	    	listh1.push({
				exam_num : item.exam_num,
