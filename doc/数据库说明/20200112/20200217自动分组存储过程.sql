...
/****** Object:  StoredProcedure [dbo].[pro_exam_autoGroup6]    Script Date: 02/17/2020 10:08:52 ******/
SET ANSI_NULLS ON
GO
...
begin
  ...
  begin
    ...
    set @strSQL = 'insert into #tmp_table(exam_info_id, group_id, exam_num, name, sex, age, is_marrige, position, other, cust_type_id) '
                + 'select e.id as examinfo_id, ' + cast(@group_id as varchar(10)) + ' as group_id, e.exam_num, c.user_name, c.sex, e.age, e.is_marriage, e.position, e.group_index, e.customer_type_id '
	            + 'from exam_info e '
	            + 'inner join customer_info c on e.customer_id=c.id '
	            + 'where e.batch_id='+ @batch_id +' and c.is_Active=''Y'' and e.is_Active=''Y'' and e.status=''Y'' and (e.group_id is null or e.group_id=0) '
