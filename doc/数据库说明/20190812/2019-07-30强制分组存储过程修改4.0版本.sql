...
/****** Object:  StoredProcedure [dbo].[pro_exam_enforceGroup]    Script Date: 08/06/2019 11:54:24 ******/
SET ANSI_NULLS ON
GO
...
begin
   ...
   begin
     ...
     begin try
      ...
      delete from examResult_chargingItem where exam_id in (select id from sample_exam_detail where exam_num=@exam_num) and result_type='sample'
	  ...
	  insert into examinfo_set(examinfo_id, exam_set_id, exam_indicator, discount, amount, is_new_added, isActive, creater, create_time, final_exam_date,app_type,exam_num)
	  select @exam_info_id, exam_set_id, 'T', s.set_discount, s.set_amount, 0, 'Y', @user_id, GETDATE(), GETDATE(),'1',@exam_num
        ...
        where ci.dep_id=d.id and ci.id=@charge_item_id
          ...
          begin
		      declare @ParamValue varchar(20),
				  @BarCode_Class int
              set @id = null
			  select @id=id from sample_exam_detail where exam_num=@exam_num and sample_id=@sam_demo_id
			  if @id is null
			  begin
			             --检验条码
