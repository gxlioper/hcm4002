...
ALTER procedure [dbo].[P_GetSysParam](  --参数读取存储过程
   @ParamId varchar(20)='exam_no', /*sys_param表中的param_id的值(exam_no:体检号；barcode：条码号；vipno：档案号; lis_order_no:lis用117);
                            ...
                            daily_acc_num ：收费员日结号，
                            fd_acc_num ：财务部门日结号, print_task_no:打印任务号
...
as
  ...
  --select @curday
  if @ParamId='print_task_no' --打印任务号
  begin
    ...
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
		...
		values('print_task_no', '打印任务号', @curValue, '打印任务号')
	  end
	  else begin
  ...
  else if @ParamId='icd_id'    --ICD10编码
  begin
    set @head='icd_'
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    ...
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('icd_id', 'ICD编码', @curValue, 'ICD编码')
	  end
	  else begin
  ...
  begin
    ...
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    set @curValue=@head+'00001'
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('com_num', '单位编码', @curValue, '单位编码')
	  end
	  else begin
  ...
  else if @ParamId='batch_num'    --任务编码(批次编码)
  begin
    ...
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    ...
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('batch_num', '批次编码', @curValue, '任务编码(批次编码)')
	  end
	  else begin
	    set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
  ...
  else if @ParamId='group_num'    --分组编码
  begin
    ...
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    set @curValue=@head+'001'
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('group_num', '分组编码', @curValue, '分组编码')
	  end
	  else begin
	    set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
	    if (@curValue=NULL) or (substring(@curValue,1,6)!=@head)
  ...
  else if @ParamId='balance_num' --团体结算单号
  begin
	  ...
	  begin
		insert into sys_param1(param_id, param_name, param_value, note)
		values('balance_num', '团体结算单号', @curValue, '用于团体结算')
	  end
	  else begin
		set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
  ...
  else if @ParamId='daily_acc_num'  --收费员日结号
  begin
      --set @curday=CONVERT(varchar(10), getdate(), 112)  --yyyymmdd
      set @curday=CONVERT(varchar(10), getdate(), 12)
	  ...
	  begin
		insert into sys_param1(param_id, param_name, param_value, note)
		values('daily_acc_num', '收费员日结号', @curValue, '收费员日结号')
	  end
	  else begin
		set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
  ...
  else if @ParamId='fd_acc_num'  --财务部门日结号
  begin
      --set @curday=CONVERT(varchar(10), getdate(), 112)  --yyyymmdd
      set @curday=CONVERT(varchar(10), getdate(), 12)
	  ...
	  begin
		insert into sys_param1(param_id, param_name, param_value, note)
		values('fd_acc_num', '财务部门日结号', @curValue, '财务部门日结号')
	  end
	  else begin
		set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
		if (@curValue=NULL) or (substring(@curValue,2,6)!=@curday)
  ...
  else if @ParamId='rcpt_num'  --缴费申请单号
  begin
      --set @curday=CONVERT(varchar(10), getdate(), 112)  --yyyymmdd
      set @curday=CONVERT(varchar(10), getdate(), 12)
    ...
    else if @ParamId='visit_num' --健康计划编号
  begin
    set @curday=CONVERT(varchar(10), getdate(), 12)
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    ...
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('visit_num', '健康计划编号', @curValue, '健康计划编号')
	  end
	  else begin
	    set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
		if (@curValue=NULL) or (substring(@curValue,1,6)!=@curday)
  ...
  else if @ParamId='sign_num' --签单计划编号
  begin
    set @curday=CONVERT(varchar(10), getdate(), 12)
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    set @curValue=@curday+'_0001'
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('sign_num', '签单计划编号', @curValue, '签单计划编号')
	  end
	  else begin
	    set @curValue=(select param_value from sys_param1 where param_id=@ParamId)
		if (@curValue=NULL) or (substring(@curValue,1,6)!=@curday)
  ...
  begin
    if not exists (select param_id from sys_param1 where param_id=@ParamId)
	  begin
	    ...
	    insert into sys_param1(param_id, param_name, param_value, note)
        values('disease_no', '疾病库知识库编码', @curValue, '疾病库知识库编码')
	  end
	  else begin
	    set @curValue=(select substring(param_value,2,7) from sys_param1 where param_id=@ParamId)
	    if (@curValue=NULL)
