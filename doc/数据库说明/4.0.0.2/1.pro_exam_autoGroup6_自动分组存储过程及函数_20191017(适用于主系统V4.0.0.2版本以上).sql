IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fun_GetCenterConfigVaule]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[fun_GetCenterConfigVaule]
GO

create function dbo.fun_GetCenterConfigVaule(
  /*************获取参数配置表值************/
  @center_num varchar(30),      --中心编码
  @config_key varchar(40)='IS_AFTER_PAY'       --参数主键
  ) returns varchar(50)
as
begin
	declare @config_value varchar(10)
    select @config_value=config_value from dbo.center_configuration where config_key='IS_AFTER_PAY' and center_num=@center_num
    return @config_value
end

go

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[pro_exam_autoGroup6]') AND type in (N'P', N'PC'))
DROP PROCEDURE dbo.pro_exam_autoGroup6
GO

create procedure [dbo].[pro_exam_autoGroup6](
  /************自动分组存储过程*****************/
...
begin
  ...
  begin
    ...
    set @strSQL = 'insert into #tmp_table(exam_info_id, group_id, exam_num, name, sex, age, is_marrige, position, other, cust_type_id) '
				...
				+ ' and not exists (select t.exam_info_id from (select e.id as exam_info_id from exam_info e, examinfo_charging_item eci '
                + ' where e.id=eci.examinfo_id and eci.isActive=''Y'' and e.batch_id='+ @batch_id +' group by e.id) as t where t.exam_info_id=e.id) '
