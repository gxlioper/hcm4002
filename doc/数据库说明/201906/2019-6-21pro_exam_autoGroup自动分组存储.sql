...
if exists(select * from sys.procedures where name='pro_exam_autoGroup')
drop procedure pro_exam_autoGroup;
go


CREATE procedure [dbo].[pro_exam_autoGroup](
  /************自动分组存储过程*****************/
         ...
         -- values(@group_id, '自动分组', @exam_info_id, @exam_num, 'examinfo_charging_item', '写入examinfo_charging_item成功， 项目ID=' + cast(@charge_item_id as varchar(20)))
          ...
          begin
		       declare @ParamValue varchar(20),
				          @BarCode_Class int
              set @id=null
			  select @id=id,@ParamValue=sample_barcode from sample_exam_detail where exam_info_id=@exam_info_id and sample_id=@sam_demo_id
			  if @id is null
			  begin
								   --检验条码
