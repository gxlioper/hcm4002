...
public class CustomerInfoAction extends BaseAction implements ModelDriven {
	...
	public com.hjw.wst.service.examInfoService getExamInfoService() {
		return examInfoService;
	}

	public void setExamInfoService(com.hjw.wst.service.examInfoService examInfoService) {
...
//					this.message = "ok-自动分组执行完成";
		...
		if (this.model.getCompany_id() <= 0) {
			...
			if (this.model.getBatch_id() <= 0) {
				...
				if (!StringUtil.isEmpty(model.getExam_num())) {
					...
					eu = this.customerInfoService.getExamInfoUserForExamId(model.getExam_num(),user);
	...
	public String editcustomerdo() throws WebException, SQLException, Exception {
		...
		if ((this.model.getCompany_id() <= 0) || (this.model.getBatch_id() <= 0)) {
			 ...
			 * this.batchService.getBatchByID(this.model.getBatch_id()); if
			...
			ExamInfoUserDTO eiold = this.customerInfoService.getExamInfoUserForExamId(this.model.getExam_num(),user);
			//eiold = this.djtCustomerInfoService.getExamInfoForId(this.model.getExam_num());
		...
		if ((this.model.getCompany_id() <= 0) || (this.model.getBatch_id() <= 0)) {
			...
			if ("1".equals(bt.getOverflag())) {
				...
				if (bt.getId() > 0) {
					...
					if ("ok".equals(this.message.split("-")[0])) {
						String exam_id = this.message.split("-")[1];
						if (this.model.getGroup_id() > 0) {
							this.customerInfoService.dotUserGroup(this.model.getGroup_id(), user.getUserid() + "",
									user.getCenter_num(), exam_id);
						}
					}
					// UserDTO user = (UserDTO) session.get("username");
					SysLog sl = new SysLog();
    ...
    public String batchSigndo() throws WebException, SQLException {
		...
		if ((this.model.getCompany_id() <= 0) || (this.model.getBatch_id() <= 0)) {
			...
			if ("1".equals(taover.getOverflag())) {
				...
				if ((!StringUtil.isEmpty(this.model.getExam_nums()))) {
					//String ids = this.model.getIds().trim().substring(1, this.model.getIds().trim().length());
					String exam_nums = this.model.getExam_nums().trim().substring(1,this.model.getExam_nums().trim().length());
					...
					if ("Y".equals(
						...
						for(int i=0;i<exam_nums_s.length;i++){
							// 可以发挂号申请
							ExamInfoDTO ei = this.customerInfoService.getExamInfoForexamNum(exam_nums_s[i]);
							if (ei != null &&"Y".equals(ei.getIs_Active())) {
								ExamInfo examInfo = examInfoService.findExamInfo(ei.getExam_num());
...
//										|| (this.model.getVisit_no().length() < 0)
									...
									if ("AA".equals(fr.getResultHeader().getTypeCode())) {
										...
										// 必须填写
										sl.setExplain("登记台人员挂号 " + examInfo.getExam_num() + " " + ei.getUser_name()
												+ " " + examInfo.getPatient_id() + " " + examInfo.getVisit_no() + " "
												+ examInfo.getVisit_date() + " " + examInfo.getClinic_no());// 操作说明
										// syslogService.saveSysLog(sl);
									}
								} else {
									WebserviceConfigurationDTO wcd = new WebserviceConfigurationDTO();
		...
		if ((this.model.getIds() == null) || (this.model.getIds().trim().length() <= 0)) {
			...
			if ("1".equals(taover.getOverflag())) {
				...
				for (int i = 0; i < exam_nums.length; i++) {
					...
					this.customerInfoService.addIDSExamInfoChangItem(exam_nums[i], listitem, itemcodes,
			...
			if (StringUtil.isEmpty(model.getExam_nums())) {
				...
				if (this.model.getId() <= 0 ) {
					...
					for (int i = 0; i < exam_nums.length; i++) {
						...
						String charging_code =  chargingItemService.findChargingItem(chargingId).getItem_code();
					...
					for (int i = 0; i < exam_nums.length; i++) {
						long chargingId = this.model.getId();
						String charging_code =  chargingItemService.findChargingItem(chargingId).getItem_code();
						this.customerInfoService.delIDSExamInfoChangItem(exam_nums[i], charging_code, user.getUserid(),user);
						// UserDTO user = (UserDTO) session.get("username");
						SysLog sl = new SysLog();
						sl.setCenter_num(user.getCenter_num());
						sl.setUserid(user.getUserid() + "");
	...
	/**
	 * 
	 * @Title: getComByBatchList @Description: 通过单位名称获取体检任务
	...
	public String getComByBatchList() throws WebException, Exception {
		...
		if(!StringUtil.isEmpty(this.q)){
			model.setComname(this.q);
		}
		tr = this.batchService.getCompanyForNameBatch(model.getComname(),this.model.getRemark(), this.defaultapp.getComid(),user);
	...
	/**
	 * 
	 * @Title: getComByBatchList @Description: 根据单位id获取所有的子单位
	...
	public String getbatchcompanyshow() throws WebException, Exception {
		...
		this.defaultapp = (SystemType) session.get("defaultapp");
		//根据单位id获取批次
		tr = this.companyService.getBatchForComId(model.getCompany_id(),this.defaultapp.getComid(),user.getCenter_num());
		this.outJsonResult(tr);
		return NONE;
	...
	public String getItemSampleDemo() throws WebException{
		UserDTO user = (UserDTO) session.get("username");
		List<GroupChargingItemDTO> li  = this.batchService.getItemSampleDemo(model.getId(), user.getCenter_num());
		...
		//团体转个人资源
				...
				if (web != null) {
					for (int i = 0; i < web.size(); i++) {
						...
						//分组冻结的资源
						if (web.get(i).getRes_code().equals("RS053")) {
							if ("1".equals(web.get(i).getDatavalue())) {
								this.model.setGrouptot(1);
								break;
							}
						}
					}
   ...
   //生成档案号
    public  String createArch_num(){
    	UserDTO user = (UserDTO) session.get("username");
    	String arch_num = this.batchService.GetCreateID("vipno", user.getCenter_num());
