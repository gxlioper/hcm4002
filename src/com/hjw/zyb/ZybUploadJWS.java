...
public class ZybUploadJWS extends HttpServlet {
	...
	private String readHSSF(String company_id, String companyname, String batch_id, String batchname, long userid,
		...
		HSSFSheet worksheet = workbook.getSheetAt(0);
			...
			if (null == firstDataRow || null == firstDataRow.getCell(1)) {
				...
				List<ZybDiseaseHistory> li = new ArrayList<ZybDiseaseHistory>();
				int rowIndex = 1;
				boolean hasRecord = true;
				while (hasRecord) {
					ZybDiseaseHistory  s = new ZybDiseaseHistory();
...
//						/.replaceAll("[　*| *| *|//s*]*", "")
					...
					if (cell != null) {
						s.setDiagnosisdate(row.getCell(2).toString());
					} else {
						s.setDiagnosisdate("");
					}
				...
				String examnum = zybOccuhisService.saveDiseaseHistory(li, userid);
